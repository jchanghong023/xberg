...
// alef:hash:54c07b5eed007d7d25bd1750d3ff65bd6fe17681b8d2e69a6c0c825483bc8efb
...
public sealed record Attributes
{
    ...
    public List<KeyValueAttribute> KeyValues { get; init; } = [];
    ...
    public static Attributes FromJson(string json)
    {
        try
        {
            return JsonSerializer.Deserialize<Attributes>(json, JsonOptions)
                ?? throw new XbergException($"Failed to parse Attributes from JSON: deserializer returned null");
        }
        ...
        {
            throw new XbergException($"Failed to parse Attributes from JSON: {e.Message}", e);
        }
    }
