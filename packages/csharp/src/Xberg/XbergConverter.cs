...
// alef:hash:8cfb25b66c9877677c8dee0b3d2dbf197fcfbc7fc0d3999f4d19b5e2ef9479c1
#nullable enable
...
public static class XbergConverter
{
    ...
    /// </summary>
    /// <exception cref="XbergException">Returns the same validation and LLM errors as `classify_chunks`.</exception>
    ...
    public static async Task<ExtractedDocument> ClassifyChunksOwnedAsync(ExtractedDocument result, ChunkClassificationConfig config)
    {
        ...
        if (configHandle == 0)
        {
            throw XbergException.FromLastError("ChunkClassificationConfigFromJson failed");
        }
        try
        {
            return await Task.Run(() =>
            {
                var nativeResult = NativeMethods.ClassifyChunksOwned(
                    resultHandle,
                    configHandle
                );
        ...
        {
            if (resultHandle != 0) NativeMethods.ExtractedDocumentFree(resultHandle);
            if (configHandle != 0) NativeMethods.ChunkClassificationConfigFree(configHandle);
        }
    }
    ...
    public static DoctorReport Doctor(ExtractionConfig config)
    {
        ...
        {
            if (configHandle != 0) NativeMethods.ExtractionConfigFree(configHandle);
        }
    }
    ...
    public static async Task<ExtractionResult> ExtractAsync(ExtractInput input, ExtractionConfig config)
    {
        ...
        {
            if (inputHandle != 0) NativeMethods.ExtractInputFree(inputHandle);
            if (configHandle != 0) NativeMethods.ExtractionConfigFree(configHandle);
        }
    }
    ...
    /// cache.
    /// </summary>
    /// <exception cref="XbergException">Returns an error only if the registry lock cannot be acquired in the current environment.</exception>
    public static List<OcrBackendCapabilities> ListOcrBackendCapabilities()
    {
        var nativeResult = NativeMethods.ListOcrBackendCapabilities();
        if (NativeMethods.LastErrorCode() != 0)
        {
            throw GetLastError();
        }
        var json = global::System.Runtime.InteropServices.Marshal.PtrToStringUTF8(nativeResult);
        NativeMethods.FreeString(nativeResult);
        var returnValue = JsonSerializer.Deserialize<List<OcrBackendCapabilities>>(json ?? "null", JsonOptions)!;
        return returnValue;
    }
    ...
    public static async Task<MapResult> MapUrlAsync(string uri, UrlExtractionConfig config)
    {
        ...
        {
            if (configHandle != 0) NativeMethods.UrlExtractionConfigFree(configHandle);
        }
    }
    ...
    public static List<LateInteractionMatch> MaxSimRank(MultiVectorEmbedding query, List<MultiVectorEmbedding> docs)
    {
        ...
        {
            if (queryHandle != 0) NativeMethods.MultiVectorEmbeddingFree(queryHandle);
    ...
    public static double MaxSimScore(MultiVectorEmbedding query, MultiVectorEmbedding doc)
    {
        ...
        {
            if (queryHandle != 0) NativeMethods.MultiVectorEmbeddingFree(queryHandle);
            if (docHandle != 0) NativeMethods.MultiVectorEmbeddingFree(docHandle);
        }
    }
    ...
    /// </summary>
    /// <param name="backend">Name of a registered OCR backend, as returned by `list_ocr_backends`. Lookup is case-insensitive and resolves the same `paddleocr` alias as backend dispatch.</param>
    ///
    /// <param name="language">Language code to check (e.g. `"eng"`, `"deu"`).</param>
    ///
    /// <exception cref="XbergException">Returns an error if no backend with that name (or alias) is registered.</exception>
    ...
    public static bool OcrBackendSupportsLanguage(string backend, string language)
    {
        ...
        var nativeResult = NativeMethods.OcrBackendSupportsLanguage(
            backend,
            language
        );
        if (NativeMethods.LastErrorCode() != 0)
    ...
    {
        var nativeResult = NativeMethods.ChunkClassificationConfigDefaultBatchSize();
        var returnValue = nativeResult;
        return returnValue;
    }
    ...
    {
        var nativeResult = NativeMethods.ChunkClassificationConfigDefaultMaxConcurrency();
        var returnValue = nativeResult;
        return returnValue;
    }
    ...
    {
        if (NativeMethods.ExtractionConfigDefaultMaxEmbeddedFileBytesHasResult() != 1)
        {
            return (ulong?)null;
        }
        var nativeResult = NativeMethods.ExtractionConfigDefaultMaxEmbeddedFileBytes();
        var returnValue = nativeResult;
        return returnValue;
    }
    ...
    {
        if (NativeMethods.ExtractionConfigDefaultExtractionTimeoutHasResult() != 1)
        {
            return (ulong?)null;
        }
    ...
    {
        var nativeResult = NativeMethods.ExtractionConfigValidate(
            handle
        );
        if (nativeResult != 0)
        {
    ...
    /// format` (GH#1662).
    /// </summary>
    public static bool ExtractionConfigRunsOcrOnEmbeddedImages(ulong handle)
    {
        var nativeResult = NativeMethods.ExtractionConfigRunsOcrOnEmbeddedImages(
            handle
        );
        var returnValue = nativeResult != 0;
        return returnValue;
    ...
    /// QR-code detection.
    /// </summary>
    public static bool ExtractionConfigWantsOwnBytesInResult(ulong handle)
    {
        var nativeResult = NativeMethods.ExtractionConfigWantsOwnBytesInResult(
            handle
        );
        var returnValue = nativeResult != 0;
        return returnValue;
    ...
    public static FootnoteConfig FootnoteConfigWithParseCitations(ulong handle, bool enabled)
    {
    ...
    {
        var nativeResult = NativeMethods.HeuristicsConfigValidate(
            handle
        );
        if (nativeResult != 0)
        {
    ...
    {
        var nativeResult = NativeMethods.LlmConfigValidate(
            handle
        );
        if (nativeResult != 0)
        {
    ...
    public static PaddleOcrConfig PaddleOcrConfigWithCacheDir(ulong handle, string path)
    {
    ...
    public static PaddleOcrConfig PaddleOcrConfigWithTableDetection(ulong handle, bool enable)
    {
    ...
    /// <param name="enable"></param>
    public static PaddleOcrConfig PaddleOcrConfigWithAngleCls(ulong handle, bool enable)
    {
    ...
    public static PaddleOcrConfig PaddleOcrConfigWithDetDbThresh(ulong handle, float threshold)
    {
    ...
    /// <param name="threshold"></param>
    public static PaddleOcrConfig PaddleOcrConfigWithDetDbBoxThresh(ulong handle, float threshold)
    {
    ...
    public static PaddleOcrConfig PaddleOcrConfigWithDetDbUnclipRatio(ulong handle, float ratio)
    {
    ...
    public static PaddleOcrConfig PaddleOcrConfigWithDetLimitSideLen(ulong handle, uint length)
    {
    ...
    public static PaddleOcrConfig PaddleOcrConfigWithRecBatchNum(ulong handle, uint batchSize)
    {
    ...
    public static PaddleOcrConfig PaddleOcrConfigWithDropScore(ulong handle, float score)
    {
    ...
    public static PaddleOcrConfig PaddleOcrConfigWithPadding(ulong handle, uint padding)
    {
    ...
    public static PaddleOcrConfig PaddleOcrConfigWithModelTier(ulong handle, string tier)
    {
    ...
    public static PaddleOcrConfig PaddleOcrConfigWithModelVersion(ulong handle, string version)
    {
    ...
    /// </summary>
    /// <exception cref="XbergException">Returns `Validation` when a configured page margin is
    ...
    /// unavailable in the current feature set.</exception>
    public static void PdfConfigValidate(ulong handle)
    {
        var nativeResult = NativeMethods.PdfConfigValidate(
            handle
        );
        if (nativeResult != 0)
        {
    ...
    {
        var nativeResult = NativeMethods.RedactionConfigValidate(
            handle
        );
        if (nativeResult != 0)
        {
    ...
    {
        var nativeResult = NativeMethods.StructuredExtractionConfigDefaultSchemaName();
        if (nativeResult == IntPtr.Zero)
        {
            throw GetLastError();
        }
    ...
    {
        var nativeResult = NativeMethods.UrlExtractionConfigDefaultXbergCrawlConfig();
        if (nativeResult == 0)
        {
            throw GetLastError();
        }
