...
// alef:hash:3404103fa7eccaf6e3a30c68afae27544647523a08d9cfcc8ea3cac4ee01ac09
    ...
    public static ImageDimensions FromJson(string json)
    {
        try
        {
            return JsonSerializer.Deserialize<ImageDimensions>(json, JsonOptions)
                ?? throw new XbergException($"Failed to parse ImageDimensions from JSON: deserializer returned null");
        }
        ...
        {
            throw new XbergException($"Failed to parse ImageDimensions from JSON: {e.Message}", e);
        }
    }
