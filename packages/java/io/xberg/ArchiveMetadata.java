// This file is auto-generated by alef — DO NOT EDIT.
// alef:hash:e4d7a628f0b5912f569a4535cbee6b5d6f4e1f286806c9fd5d9193f8fe391c71
// To regenerate: alef generate
// To verify freshness: alef verify
package io.xberg;

import com.fasterxml.jackson.annotation.JsonInclude;
import com.fasterxml.jackson.annotation.JsonProperty;
import com.fasterxml.jackson.databind.annotation.JsonDeserialize;
import com.fasterxml.jackson.databind.annotation.JsonPOJOBuilder;
import java.util.List;
import org.jspecify.annotations.Nullable;

/**
 * Archive (ZIP/TAR/7Z) metadata.
 *
 * Extracted from compressed archive files containing file lists and size
 * information.
 */
@JsonInclude(JsonInclude.Include.NON_ABSENT)
@JsonDeserialize(builder = ArchiveMetadata.Builder.class)
public record ArchiveMetadata(@JsonProperty("format") String format,
                              @JsonProperty("file_count") int fileCount,
                              @JsonProperty("file_list") List<String> fileList,
                              @JsonProperty("total_size") long totalSize,
                              @Nullable @JsonProperty("compressed_size")
                              Long compressedSize) {
  /** Creates a new Builder for constructing instances of this record. */
  public static Builder builder() { return new Builder(); }

  // CPD-OFF
  /** Jackson builder for ArchiveMetadata deserialization. */
  @com.fasterxml.jackson.annotation.JsonIgnoreProperties(ignoreUnknown = true)
  @JsonPOJOBuilder(withPrefix = "with", buildMethodName = "build")
  public static final class Builder {

    private String format;
    @JsonProperty("file_count") private int fileCount;
    @JsonProperty("file_list") private List<String> fileList = List.of();
    @JsonProperty("total_size") private long totalSize;
    @JsonProperty("compressed_size") @Nullable private Long compressedSize;

    /** Sets the format field. */
    @JsonProperty("format")
    public Builder withFormat(final String value) {
      this.format = value;
      return this;
    }

    /** Sets the fileCount field. */
    @JsonProperty("file_count")
    public Builder withFileCount(final int value) {
      this.fileCount = value;
      return this;
    }

    /** Sets the fileList field. */
    @JsonProperty("file_list")
    public Builder withFileList(final List<String> value) {
      this.fileList = value;
      return this;
    }

    /** Sets the totalSize field. */
    @JsonProperty("total_size")
    public Builder withTotalSize(final long value) {
      this.totalSize = value;
      return this;
    }

    /** Sets the compressedSize field. */
    @JsonProperty("compressed_size")
    public Builder withCompressedSize(final @Nullable long value) {
      this.compressedSize = value;
      return this;
    }

    /**
     * Constructs a ArchiveMetadata instance from the builder's current state.
     */
    public ArchiveMetadata build() {
      return new ArchiveMetadata(format, fileCount, fileList, totalSize,
                                 compressedSize);
    }
  }
  // CPD-ON
}
