...
// alef:hash:c15e10e121315a599442ab02ce2353e03c4c88ae103f77f7e5e50aef6a74fc8b
...
public final class PostProcessorBridge implements AutoCloseable {
  ...
  private static final System.Logger LOGGER =
      System.getLogger(PostProcessorBridge.class.getName());
  ...
  // C vtable: 11 fields (4 plugin methods + 5 trait methods + free_string +
  // free_user_data)
  ...
  private void initStubProcess(long offset)
      throws ReflectiveOperationException {
  ...
  private void initStubProcessingStage(long offset)
      throws ReflectiveOperationException {
  ...
  private void initStubShouldProcess(long offset)
      throws ReflectiveOperationException {
  ...
  private void initStubEstimatedDurationMs(long offset)
      throws ReflectiveOperationException {
  ...
  private void initStubPriority(long offset)
      throws ReflectiveOperationException {
  ...
  private int handleProcess(MemorySegment userData, MemorySegment result_in,
                            MemorySegment config_in, MemorySegment outError) {
    try {
      String result_json = result_in.reinterpret(Long.MAX_VALUE).getString(0);
      ExtractedDocument result =
          JSON.readValue(result_json, ExtractedDocument.class);
      String config_json = config_in.reinterpret(Long.MAX_VALUE).getString(0);
      ExtractionConfig config =
          JSON.readValue(config_json, ExtractionConfig.class);
  ...
  private int handleProcessingStage(MemorySegment userData,
  ...
  // so a host exception cannot propagate — log it before substituting the
  // default, which would otherwise be indistinguishable from a real result.
  private long handleEstimatedDurationMs(MemorySegment userData,
                                         MemorySegment _result_in) {
    try {
      String _result_json = _result_in.reinterpret(Long.MAX_VALUE).getString(0);
      ExtractedDocument _result =
  ...
  public static void unregisterPostProcessor(String name) throws Exception {
    try {
      try (var nameArena = Arena.ofShared()) {
        ...
        MemorySegment outErr = nameArena.allocate(ValueLayout.ADDRESS);
        int rc = (int)(long)NativeLib.XBERG_UNREGISTER_POST_PROCESSOR.invoke(
            nameCs, outErr);
        if (rc != 0) {
          MemorySegment errPtr = outErr.get(ValueLayout.ADDRESS, 0);
          String msg = errPtr.equals(MemorySegment.NULL)
