// This file is auto-generated by alef — DO NOT EDIT.
// alef:hash:4fb43cc9bc315e63d7f4bcb5f99ba009c6ce0cea317d6c0260c7b0cd02a009f4
// To regenerate: alef generate
// To verify freshness: alef verify
package io.xberg;

import com.fasterxml.jackson.annotation.JsonInclude;
import com.fasterxml.jackson.annotation.JsonProperty;
import com.fasterxml.jackson.databind.annotation.JsonDeserialize;
import com.fasterxml.jackson.databind.annotation.JsonPOJOBuilder;
import org.jspecify.annotations.Nullable;

/**
 * Cross-extractor content filtering configuration.
 *
 * Controls whether "furniture" content (headers, footers, page numbers,
 * watermarks, repeating text) is included in or stripped from extraction
 * results. Applies across all extractors (PDF, DOCX, RTF, ODT, HTML, etc.)
 * with format-specific implementation.
 *
 * When {@code None} on {@code ExtractionConfig}, each extractor uses its
 * current default behavior unchanged.
 */
@JsonInclude(JsonInclude.Include.NON_ABSENT)
@JsonDeserialize(builder = ContentFilterConfig.Builder.class)
public record ContentFilterConfig(
    @Nullable @JsonProperty("include_headers") Boolean includeHeaders,
    @Nullable @JsonProperty("include_footers") Boolean includeFooters,
    @Nullable @JsonProperty("include_footnotes") Boolean includeFootnotes,
    @Nullable @JsonProperty("strip_repeating_text") Boolean stripRepeatingText,
    @Nullable @JsonProperty("include_watermarks") Boolean includeWatermarks) {
  /** Creates a new Builder for constructing instances of this record. */
  public static Builder builder() { return new Builder(); }
  public ContentFilterConfig {
    if (stripRepeatingText == null) {
      stripRepeatingText = true;
    }
  }

  // CPD-OFF
  /** Jackson builder for ContentFilterConfig deserialization. */
  @com.fasterxml.jackson.annotation.JsonIgnoreProperties(ignoreUnknown = true)
  @JsonPOJOBuilder(withPrefix = "with", buildMethodName = "build")
  public static final class Builder {

    @JsonProperty("include_headers") @Nullable private Boolean includeHeaders;
    @JsonProperty("include_footers") @Nullable private Boolean includeFooters;
    @JsonProperty("include_footnotes")
    @Nullable
    private Boolean includeFootnotes;
    @JsonProperty("strip_repeating_text")
    @Nullable
    private Boolean stripRepeatingText;
    @JsonProperty("include_watermarks")
    @Nullable
    private Boolean includeWatermarks;

    /** Sets the includeHeaders field. */
    @JsonProperty("include_headers")
    public Builder withIncludeHeaders(final @Nullable Boolean value) {
      this.includeHeaders = value;
      return this;
    }

    /** Sets the includeFooters field. */
    @JsonProperty("include_footers")
    public Builder withIncludeFooters(final @Nullable Boolean value) {
      this.includeFooters = value;
      return this;
    }

    /** Sets the includeFootnotes field. */
    @JsonProperty("include_footnotes")
    public Builder withIncludeFootnotes(final @Nullable Boolean value) {
      this.includeFootnotes = value;
      return this;
    }

    /** Sets the stripRepeatingText field. */
    @JsonProperty("strip_repeating_text")
    public Builder withStripRepeatingText(final @Nullable Boolean value) {
      this.stripRepeatingText = value;
      return this;
    }

    /** Sets the includeWatermarks field. */
    @JsonProperty("include_watermarks")
    public Builder withIncludeWatermarks(final @Nullable Boolean value) {
      this.includeWatermarks = value;
      return this;
    }

    /**
     * Constructs a ContentFilterConfig instance from the builder's current
     * state.
     */
    public ContentFilterConfig build() {
      return new ContentFilterConfig(includeHeaders, includeFooters,
                                     includeFootnotes, stripRepeatingText,
                                     includeWatermarks);
    }
  }
  // CPD-ON
}
