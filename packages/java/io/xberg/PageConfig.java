// This file is auto-generated by alef — DO NOT EDIT.
// alef:hash:e247d0be076a4e55c457a6e82cb335fd9a1b8f949f87c576c3be4fcaaceecca7
// To regenerate: alef generate
// To verify freshness: alef verify
package io.xberg;

import com.fasterxml.jackson.annotation.JsonInclude;
import com.fasterxml.jackson.annotation.JsonProperty;
import com.fasterxml.jackson.databind.annotation.JsonDeserialize;
import com.fasterxml.jackson.databind.annotation.JsonPOJOBuilder;
import org.jspecify.annotations.Nullable;

/**
 * Page extraction and tracking configuration.
 *
 * Controls how pages are extracted, tracked, and represented in the extraction
 * results. When {@code None}, page tracking is disabled.
 *
 * Page range tracking in chunk metadata (first_page/last_page) is automatically
 * enabled when page boundaries are available and chunking is configured.
 */
@JsonInclude(JsonInclude.Include.NON_ABSENT)
@JsonDeserialize(builder = PageConfig.Builder.class)
public record PageConfig(
    @Nullable @JsonProperty("extract_pages") Boolean extractPages,
    @Nullable @JsonProperty("insert_page_markers") Boolean insertPageMarkers,
    @Nullable @JsonProperty("marker_format") String markerFormat) {
  /** Creates a new Builder for constructing instances of this record. */
  public static Builder builder() { return new Builder(); }

  // CPD-OFF
  /** Jackson builder for PageConfig deserialization. */
  @com.fasterxml.jackson.annotation.JsonIgnoreProperties(ignoreUnknown = true)
  @JsonPOJOBuilder(withPrefix = "with", buildMethodName = "build")
  public static final class Builder {

    @JsonProperty("extract_pages") @Nullable private Boolean extractPages;
    @JsonProperty("insert_page_markers")
    @Nullable
    private Boolean insertPageMarkers;
    @JsonProperty("marker_format") @Nullable private String markerFormat;

    /** Sets the extractPages field. */
    @JsonProperty("extract_pages")
    public Builder withExtractPages(final @Nullable Boolean value) {
      this.extractPages = value;
      return this;
    }

    /** Sets the insertPageMarkers field. */
    @JsonProperty("insert_page_markers")
    public Builder withInsertPageMarkers(final @Nullable Boolean value) {
      this.insertPageMarkers = value;
      return this;
    }

    /** Sets the markerFormat field. */
    @JsonProperty("marker_format")
    public Builder withMarkerFormat(final @Nullable String value) {
      this.markerFormat = value;
      return this;
    }

    /** Constructs a PageConfig instance from the builder's current state. */
    public PageConfig build() {
      return new PageConfig(extractPages, insertPageMarkers, markerFormat);
    }
  }
  // CPD-ON
}
