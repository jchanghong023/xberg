...
// alef:hash:496953b8070832714aeea8b2dcf9ca695382795e57584346e1860586aa8564da
...
public final class DocumentExtractorBridge implements AutoCloseable {
  ...
  private static final System.Logger LOGGER =
      System.getLogger(DocumentExtractorBridge.class.getName());
  ...
  // C vtable: 10 fields (4 plugin methods + 4 trait methods + free_string +
  // free_user_data)
  ...
  private void initStubExtract(long offset)
      throws ReflectiveOperationException {
    var stubExtract = LINKER.upcallStub(
        LOOKUP.bind(
            this, "handleExtract",
            MethodType.methodType(int.class, MemorySegment.class,
  ...
  private void initStubSupportedMimeTypes(long offset)
      throws ReflectiveOperationException {
  ...
  private void initStubCanHandle(long offset)
      throws ReflectiveOperationException {
  ...
  private int handleExtract(MemorySegment userData, MemorySegment input_in,
                            MemorySegment config_in, MemorySegment outResult,
                            MemorySegment outError) {
    try {
  ...
  private int handleSupportedMimeTypes(MemorySegment userData,
  ...
  // so a host exception cannot propagate — log it before substituting the
  // default, which would otherwise be indistinguishable from a real result.
  private long handleCanHandle(MemorySegment userData, MemorySegment _path_in,
    ...
    try {
      ...
      String _mime_type =
          _mime_type_in.reinterpret(Long.MAX_VALUE).getString(0);
  ...
  public static void unregisterDocumentExtractor(String name) throws Exception {
    try {
      try (var nameArena = Arena.ofShared()) {
        ...
        MemorySegment outErr = nameArena.allocate(ValueLayout.ADDRESS);
        int rc =
            (int)(long)NativeLib.XBERG_UNREGISTER_DOCUMENT_EXTRACTOR.invoke(
                nameCs, outErr);
        if (rc != 0) {
          MemorySegment errPtr = outErr.get(ValueLayout.ADDRESS, 0);
          String msg = errPtr.equals(MemorySegment.NULL)
  ...
  public static void clearDocumentExtractors() throws Exception {
    try {
      try (var arena = Arena.ofShared()) {
        ...
        int rc =
            (int)(long)NativeLib.XBERG_CLEAR_DOCUMENT_EXTRACTOR.invoke(outErr);
        if (rc != 0) {
          MemorySegment errPtr = outErr.get(ValueLayout.ADDRESS, 0);
          String msg = errPtr.equals(MemorySegment.NULL)
                           ? "clear failed (rc=" + rc + ")"
