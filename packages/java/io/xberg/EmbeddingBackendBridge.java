...
// alef:hash:e80bef09176ba48a7f505fd7faf42ab9458fa36eb6c2d24c1cd223a62e7b91cd
...
public final class EmbeddingBackendBridge implements AutoCloseable {
  ...
  private static final System.Logger LOGGER =
      System.getLogger(EmbeddingBackendBridge.class.getName());
  ...
  // C vtable: 8 fields (4 plugin methods + 2 trait methods + free_string +
  // free_user_data)
  ...
  private void initStubDimensions(long offset)
      throws ReflectiveOperationException {
  ...
  public static void unregisterEmbeddingBackend(String name) throws Exception {
    try {
      try (var nameArena = Arena.ofShared()) {
        ...
        MemorySegment outErr = nameArena.allocate(ValueLayout.ADDRESS);
        int rc = (int)(long)NativeLib.XBERG_UNREGISTER_EMBEDDING_BACKEND.invoke(
            nameCs, outErr);
        if (rc != 0) {
          MemorySegment errPtr = outErr.get(ValueLayout.ADDRESS, 0);
          String msg = errPtr.equals(MemorySegment.NULL)
  ...
  public static void clearEmbeddingBackends() throws Exception {
    try {
      try (var arena = Arena.ofShared()) {
        ...
        int rc =
            (int)(long)NativeLib.XBERG_CLEAR_EMBEDDING_BACKEND.invoke(outErr);
        if (rc != 0) {
          MemorySegment errPtr = outErr.get(ValueLayout.ADDRESS, 0);
          String msg = errPtr.equals(MemorySegment.NULL)
                           ? "clear failed (rc=" + rc + ")"
