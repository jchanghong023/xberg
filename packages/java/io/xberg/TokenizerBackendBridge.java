...
// alef:hash:c0fbddadbd53ca666b4f1abea882be509801ee64c2d3f8c01e84d4e1b5215056
...
public final class TokenizerBackendBridge implements AutoCloseable {
  ...
  private static final System.Logger LOGGER =
      System.getLogger(TokenizerBackendBridge.class.getName());
  ...
  private void initStubCountTokens(long offset)
      throws ReflectiveOperationException {
    var stubCountTokens = LINKER.upcallStub(
        LOOKUP.bind(this, "handleCountTokens",
  ...
  private long handleCountTokens(MemorySegment userData,
                                 MemorySegment text_in) {
    try {
  ...
  public static void unregisterTokenizerBackend(String name) throws Exception {
    try {
      try (var nameArena = Arena.ofShared()) {
        ...
        MemorySegment outErr = nameArena.allocate(ValueLayout.ADDRESS);
        int rc = (int)(long)NativeLib.XBERG_UNREGISTER_TOKENIZER_BACKEND.invoke(
            nameCs, outErr);
        if (rc != 0) {
          MemorySegment errPtr = outErr.get(ValueLayout.ADDRESS, 0);
          String msg = errPtr.equals(MemorySegment.NULL)
  ...
  public static void clearTokenizerBackends() throws Exception {
    try {
      try (var arena = Arena.ofShared()) {
        ...
        int rc =
            (int)(long)NativeLib.XBERG_CLEAR_TOKENIZER_BACKEND.invoke(outErr);
        if (rc != 0) {
          MemorySegment errPtr = outErr.get(ValueLayout.ADDRESS, 0);
          String msg = errPtr.equals(MemorySegment.NULL)
                           ? "clear failed (rc=" + rc + ")"
