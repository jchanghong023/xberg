...
// alef:hash:dc75b6bf5d3e4a87e8724e6e1d3250e0c45d44899f6bbe425eaed93934d58a23
...
public final class RerankerBackendBridge implements AutoCloseable {
  ...
  private static final System.Logger LOGGER =
      System.getLogger(RerankerBackendBridge.class.getName());
  ...
  // C vtable: 7 fields (4 plugin methods + 1 trait methods + free_string +
  // free_user_data)
  ...
  private void initStubRerank(long offset) throws ReflectiveOperationException {
    var stubRerank = LINKER.upcallStub(
        LOOKUP.bind(
            this, "handleRerank",
            MethodType.methodType(int.class, MemorySegment.class,
  ...
  private int handleRerank(MemorySegment userData, MemorySegment query_in,
                           MemorySegment documents_in, MemorySegment outResult,
                           MemorySegment outError) {
    try {
      String query = query_in.reinterpret(Long.MAX_VALUE).getString(0);
      String documents_json =
          documents_in.reinterpret(Long.MAX_VALUE).getString(0);
      List<String> documents = JSON.readValue(
          documents_json,
  ...
  public static void unregisterRerankerBackend(String name) throws Exception {
    try {
      try (var nameArena = Arena.ofShared()) {
        ...
        MemorySegment outErr = nameArena.allocate(ValueLayout.ADDRESS);
        int rc = (int)(long)NativeLib.XBERG_UNREGISTER_RERANKER_BACKEND.invoke(
            nameCs, outErr);
        if (rc != 0) {
          MemorySegment errPtr = outErr.get(ValueLayout.ADDRESS, 0);
          String msg = errPtr.equals(MemorySegment.NULL)
  ...
  public static void clearRerankerBackends() throws Exception {
    try {
      try (var arena = Arena.ofShared()) {
        ...
        int rc =
            (int)(long)NativeLib.XBERG_CLEAR_RERANKER_BACKEND.invoke(outErr);
        if (rc != 0) {
          MemorySegment errPtr = outErr.get(ValueLayout.ADDRESS, 0);
          String msg = errPtr.equals(MemorySegment.NULL)
                           ? "clear failed (rc=" + rc + ")"
