...
def _language_sidecar() -> dict[str, str]:
    """Load the per-document language sidecar, or an empty map when it is absent.

    `ground_truth/languages.json` is generated by `build_language_sidecar.py` (issue #1696's
    corpus work) and is absent from most checkouts: it is an opt-in artifact the language
    calibration flow produces on demand, not part of the base acquire/normalize/gate pipeline.
    Cached because `_fixture` is called once per accepted doc, and re-reading + re-parsing the
    same JSON file per doc would be pure overhead. ~keep
    """
    path = TEST_DOCS / "ground_truth" / "languages.json"
    if not path.exists():
        return {}
    try:
        return json.loads(path.read_text(encoding="utf-8")).get("languages", {})
...
def _fixture(r: dict, nbytes: int, prefix: str) -> dict:
    ...
    language = _language_sidecar().get(r["id"])
