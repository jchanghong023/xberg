...
- **(pdf): a render pass no longer reports "glyph ink is missing" for font-load warnings that drop nothing.** Any non-image engine warning captured during a page render was reported as a dropped glyph, which misclassified the Type 3 glyph-name fallback and the GH#1795 encoding-dictionary warning -- both fire during font loading and neither drops any ink. The classifier now recognizes font-load diagnostics by their structured error code (or, for the encoding-dictionary case, by its otherwise-unclassifiable message) and reports them without the false content-loss claim; a warning that does record a dropped glyph is unaffected. (GH#1794)
...
- **(pdf): a page whose text is painted by an unmapped Type 3 font routes to OCR instead of returning symbols.** A Type 3 font with no `/ToUnicode` whose `/Differences` names do not resolve through the Adobe Glyph List was reported as safely mapped, because fallback provenance was only ever raised for composite fonts. Its glyphs decode through a raw-codepoint fallback, so the page produced a plausible-looking string of symbols with a quality score of 1.0 and an extraction method of native. Such fonts now carry fallback provenance, so a page they dominate is counted as fabricated and sent to recognition. A page where the font paints only a small fragment under a longer readable body stays native, as before. (GH#1782)
