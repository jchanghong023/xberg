...
**Installation:** Tesseract language packs are installed via your OS package manager. Install the base Tesseract, then add individual language packs as needed. See the [OCR guide](/guides/ocr/#tesseract) for installation steps by platform.
...
**Selection:** Use `--ocr-language` or config `ocr.language` with a single code or list. Join multiple codes with `+` for CLI: `--ocr-language en+de+zh_hans`.
...
Sceptre implements the complete EasyOCR Gen2 recognizer family. Xberg accepts the raw model-group aliases
`english`, `latin`, `chinese_simplified`, `japanese`, `korean`, `cyrillic`, `telugu`, and `kannada`. It also accepts
the exact EasyOCR tokens and the additional ISO aliases below. A language is supported only when it appears here;
sharing a script with a model does not make an arbitrary language code valid.

| Group | Official EasyOCR Gen2 tokens | Additional aliases accepted by Xberg |
| --- | --- | --- |
| `english` | `en` | `eng` |
| `latin` | `af`, `az`, `bs`, `cs`, `cy`, `da`, `de`, `es`, `et`, `fr`, `ga`, `hr`, `hu`, `id`, `is`, `it`, `ku`, `la`, `lt`, `lv`, `mi`, `ms`, `mt`, `nl`, `no`, `oc`, `pi`, `pl`, `pt`, `ro`, `rs_latin`, `sk`, `sl`, `sq`, `sv`, `sw`, `tl`, `tr`, `uz`, `vi` | `afr`, `aze`, `bos`, `ces`/`cze`, `cym`/`wel`, `dan`, `deu`/`ger`, `spa`, `est`, `fra`/`fre`, `gle`, `hrv`, `hun`, `ind`, `isl`/`ice`, `ita`, `kur`, `lat`, `lit`, `lav`, `mri`/`mao`, `msa`/`may`, `mlt`, `nld`/`dut`, `nor`, `oci`, `pli`, `pol`, `por`, `ron`/`rum`, `sr-Latn`/`sr_Latn`, `srp-Latn`/`srp_Latn`, `slk`/`slo`, `slv`, `sqi`/`alb`, `swe`, `swa`, `fil`, `tur`, `uzb`, `vie` |
| `chinese_simplified` | `ch_sim` | `zh`, `zh-CN`, `zh-Hans`, `zho`, `chi`, `chs` |
| `japanese` | `ja` | `jpn`, `jpn_vert` |
| `korean` | `ko` | `kor` |
| `cyrillic` | `ru`, `rs_cyrillic`, `be`, `bg`, `uk`, `mn`, `abq`, `ady`, `kbd`, `ava`, `dar`, `inh`, `che`, `lbe`, `lez`, `tab`, `tjk` | `rus`, `sr-Cyrl`/`sr_Cyrl`, `srp-Cyrl`/`srp_Cyrl`, `bel`, `bul`, `ukr`, `mon`, `tg`/`tgk` |
| `telugu` | `te` | `tel` |
| `kannada` | `kn` | `kan` |

Every non-English Gen2 recognizer includes English characters. You may therefore configure English with one other
group. Do not combine two distinct non-English groups in one reader.

Desktop and server builds use ONNX Runtime and download checksum-pinned models into the Hugging Face cache. Android
and iOS use tract and exclude the downloader: the application must resolve packaged assets to filesystem paths and set
both `backend_options.model.detector_path` and `backend_options.model.recognizer_path`. The opt-in Sceptre worker
build/API requires the JavaScript host to fetch and supply the CRAFT and selected recognizer model bytes, which the
Rust engine verifies against the registry digests;
Sceptre is excluded from the default `wasm-target` bundle.
...
2. **Inline JSON config:** `--config-json '{"ocr": {"language": ["eng", "deu"]}}'`
...
- [OCR Guide](/guides/ocr/) — Backend setup, configuration, and best practices
- [Configuration Reference](/reference/configuration/) — Full list of extraction config options
- [Supported Formats](/reference/formats/) — Document format support matrix
