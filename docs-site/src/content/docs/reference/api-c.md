...
<!-- alef:hash:e673b4aae4709e8fbefa2a415c7975c921b8987b3ce0e846dc3d22abfa9ed976 -->
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `model` | `XBERGAlefHandle` | `default_balanced_embedding_model()` | The embedding model to use (defaults to "gte-modernbert-base" preset if not specified) |
...
| `max_archive_depth` | `uintptr_t` | `xberg::ExtractionConfig::default_archive_depth()` | Maximum recursion depth for archive extraction (default: 3). Set to 0 to disable recursive extraction (legacy behavior). |
...
| `class_prefix` | `const char*` | `default_class_prefix()` | CSS class prefix applied to every emitted class name. Default: `"kb-"`. Change this if your host application already uses classes that start with `kb-`. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `model` | `XBERGAlefHandle` | `default_late_interaction_model()` | The late-interaction model to use (defaults to the "gte-moderncolbert" preset). |
| `batch_size` | `uintptr_t` | `default_batch_size()` | Batch size for local ONNX inference. ColBERT emits a `[seq, dim]` multi-vector embedding per document, so memory scales with batch size — keep this modest. |
| `max_length` | `uintptr_t` | `default_max_length()` | Maximum token sequence length for the tokenizer (documents). |
| `query_max_length` | `uintptr_t` | `default_query_max_length()` | Fixed padded length for query augmentation. ColBERT queries are padded (with the mask token, kept attention-live) to exactly this many tokens rather than truncated/left as-is — this is the "query augmentation" trick from the ColBERT paper. |
...
| `max_embed_duration_secs` | `uint64_t*` | `default_max_embed_duration_secs()` | Maximum wall-clock duration (in seconds) for a single embed call when using `LateInteractionModelType.Plugin`. `NULL` disables the timeout. |
...
| `backend` | `const char*` | `default_tesseract_backend()` | OCR backend: tesseract, paddleocr, paddle-ocr, sceptre, or vlm. Sceptre uses ONNX Runtime on desktop/server and tract on supported mobile builds. Browser WebAssembly uses the separate byte-fed Sceptre worker API. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `model` | `XBERGAlefHandle` | `default_reranker_model()` | The reranker model to use (defaults to "balanced" preset if not specified). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `host` | `const char*` | `default_host()` | Server host address (e.g., "127.0.0.1", "0.0.0.0") |
| `port` | `uint16_t` | `default_port()` | Server port number |
...
| `max_request_body_bytes` | `uintptr_t` | `default_max_request_body_bytes()` | Maximum size of request body in bytes (default: 100 MB) |
| `max_multipart_field_bytes` | `uintptr_t` | `default_max_multipart_field_bytes()` | Maximum size of multipart fields in bytes (default: 100 MB) |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `model` | `XBERGAlefHandle` | `default_sparse_model()` | The sparse-embedding model to use (defaults to the "opensearch-v3-distill" preset). |
| `batch_size` | `uintptr_t` | `default_batch_size()` | Batch size for local ONNX inference. SPLADE emits a `[seq, vocab]` logit tensor per document, so memory scales with batch size — keep this modest. |
| `max_length` | `uintptr_t` | `default_max_length()` | Maximum token sequence length for the tokenizer. |
...
| `max_embed_duration_secs` | `uint64_t*` | `default_max_embed_duration_secs()` | Maximum wall-clock duration (in seconds) for a single embed call when using `SparseEmbeddingModelType.Plugin`. `NULL` disables the timeout. |
...
| `scheme_allowlist` | `const char*` | `default_scheme_allowlist()` | Allowed URI schemes. Default: `["http", "https"]`. Only `http` and `https` are supported. An empty list denies every URL. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `mode` | `const char*` | `default_reduction_mode()` | Reduction mode: "off", "light", "moderate", "aggressive", "maximum" |
...
| `max_duration_ms` | `uint64_t*` | `default_max_duration_ms()` | Hard safety limit on input duration (milliseconds). Files longer than this are rejected after decode, before model work. Default: 30 minutes. Set to `NULL` to disable (not recommended for untrusted input). |
| `max_bytes` | `uint64_t*` | `default_max_bytes()` | Hard safety limit on input size (bytes). Default: 512 MiB. Protects against pathological or malicious uploads. |
| `timeout_ms` | `uint64_t*` | `default_timeout_ms()` | Wall-clock timeout for the entire transcription operation (ms). Bounds audio decode, model resolution/download, and inference together. On expiry the extraction fails with an `XbergError.Transcription`. `NULL` disables the bound and lets the operation run unbounded (not recommended for untrusted input). Enforced on the async extraction path only; the size and duration caps (`max_bytes`, `max_duration_ms`) are checked on every path. Default: 10 minutes. |
...
| `XBERG_ON_LOW_QUALITY` | Try the classical OCR backend first. If the quality score is below `quality_threshold`, send the page to the VLM. `quality_threshold` is in the `[0.0, 1.0]` range produced by `text.quality.calculate_quality_score`. A value of `0.5` is a reasonable starting point; calibrate with the Stage 0 benchmark harness. — Fields: `quality_threshold`: `double` |
