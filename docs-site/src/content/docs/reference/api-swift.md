...
<!-- alef:hash:7735ee8fa08080a228a2a43915aa5dca9dd598a64f9acff8a4794bfd272f65e4 -->
<!-- To regenerate: alef docs -->
<!-- To verify freshness: alef verify -->

## Swift API Reference <span class="version-badge">v1.1.0</span>
...
| `maxEmbedDurationSecs` | `UInt64?` | `default_max_embed_duration_secs()` | Maximum wall-clock duration (in seconds) for a single embed call when using `LateInteractionModelType.Plugin`. `null` disables the timeout. |
...
| `maxEmbedDurationSecs` | `UInt64?` | `default_max_embed_duration_secs()` | Maximum wall-clock duration (in seconds) for a single embed call when using `SparseEmbeddingModelType.Plugin`. `null` disables the timeout. |
...
| `maxDurationMs` | `UInt64?` | `default_max_duration_ms()` | Hard safety limit on input duration (milliseconds). Files longer than this are rejected after decode, before model work. Default: 30 minutes. Set to `null` to disable (not recommended for untrusted input). |
