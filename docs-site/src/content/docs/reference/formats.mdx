...
Xberg supports 110 file formats across 146 unique file extensions and accepts 58 compatibility MIME aliases.
...
PDF extraction uses the xberg-native-pdf provider. Legacy `.doc` and `.ppt` extraction no longer requires LibreOffice; those formats are parsed through native OLE/CFB extractors.
...
| ------------------------ | --------------------------------------------------------------------------- | --------------------------------------------------------------------------- | ----------------------- | ------------------------- | ----------------------------------------------------------- |
| PDF                      | `.pdf`                                                                      | `application/pdf`                                                           | Native Rust (xberg-native-pdf) | Yes                       | Metadata extraction, image extraction, text layer detection |
| Excel                    | `.xlsx`, `.xlsm`, `.xlsb`, `.xls`, `.xlam`, `.xla`, `.xltm`, `.xltx`, `.xlt`, `.ods` | Various Excel MIME types                                                    | Native Rust (calamine)  | No                        | Multi-sheet support, formula preservation                   |
...
| PowerPoint (Legacy)      | `.ppt`, `.pps`                                                              | `application/vnd.ms-powerpoint`                                             | Native OLE/CFB          | Yes                       | Direct binary parsing                                       |
...
| dBASE                    | `.dbf`                                                                      | `application/vnd.dbf`                                                       | Native Rust (dbase)     | No                        | Table data extraction, field type support                   |
...
| Visio                    | `.vsd`, `.vsdx`, `.vsdm`                                                    | `application/vnd.visio`, `application/vnd.ms-visio.drawing`                 | Native Rust (OLE/CFB + OPC) | No                        | Shape text from legacy binary `.vsd` and OPC `.vsdx`/`.vsdm` drawing packages |
...
| CommonMark       | `.commonmark`      | `text/x-commonmark`                  | Native Rust (pulldown-cmark)                                                     | No          | Standard CommonMark parsing                                                       |
| Quarto           | `.qmd`             | `text/x-quarto`                      | Native Rust (pulldown-cmark)                                                     | No          | Quarto Markdown content                                                           |
| R Markdown       | `.rmd`             | `text/x-r-markdown`                  | Native Rust (pulldown-cmark)                                                     | No          | R Markdown content                                                                |
| HTML             | `.html`, `.htm`    | `text/html`                          | Native Rust ([html-to-markdown-rs](https://docs.html-to-markdown.xberg.io)) | No          | Converts to Markdown, metadata extraction                                        |
| XHTML            | `.xhtml`, `.xht`   | `application/xhtml+xml`              | Native Rust ([html-to-markdown-rs](https://docs.html-to-markdown.xberg.io)) | No          | XML-serialized HTML with Markdown conversion                                     |
| XML              | `.xml`             | `application/xml`, `text/xml`        | Native Rust (quick-xml streaming)                                                | No          | Element counting, unique element tracking                                        |
| KML              | `.kml`             | `application/vnd.google-earth.kml+xml` | Native Rust (quick-xml streaming)                                              | No          | Geographic feature and metadata extraction                                       |
| SVG              | `.svg`             | `image/svg+xml`                      | Native Rust (XML parser)                                                         | No          | Treated as XML document                                                          |
| reStructuredText | `.rst`             | `text/prs.fallenstein.rst`           | Native (rst-parser)                                                              | No          | Full reST syntax support                                                         |
| Org Mode         | `.org`             | `text/org`                           | Native (org)                                                                     | No          | Emacs Org mode support                                                           |
| Rich Text Format | `.rtf`             | `application/rtf`, `text/rtf`        | Native (rtf-parser)                                                              | No          | RTF 1.x support                                                                  |
| Djot             | `.djot`, `.dj`     | `text/x-djot`                        | Native Rust (jotdown)                                                            | No          | Smart punctuation, tables, code blocks, YAML frontmatter, footnotes, math blocks |
| MDX              | `.mdx`             | `text/mdx`                           | Native Rust (pulldown-cmark)                                                     | No          | JSX-in-Markdown, component-based documents                                       |
| Docling DocTags  | `.doctags`         | `text/vnd.docling.doctags`           | Native Rust                                                                      | No          | Explicit DocTags representation                                                   |
...
| GeoJSON | `.geojson`     | `application/geo+json`                           | Native Rust (serde_json) | No          | Geographic feature and geometry extraction  |
| JSON Lines | `.jsonl`, `.ndjson` | `application/x-ndjson`                       | Native Rust (serde_json) | No          | Newline-delimited JSON records              |
| YAML   | `.yaml`, `.yml` | `application/yaml`, `application/x-yaml`, `text/yaml`, `text/x-yaml` | Native Rust (serde_yaml) | No          | Multi-document support, field counting      |
...
| SQLite | `.sqlite`, `.sqlite3`, `.db` | `application/vnd.sqlite3`            | Native Rust (`sqlite`)   | No          | User tables; internal metadata excluded; BLOB size placeholders |
| GeoPackage | `.gpkg`, `.gpkx` | `application/geopackage+sqlite3`             | Native Rust (`sqlite`)   | No          | Application-ID detection; user tables without GeoPackage metadata |
...
| JPEG 2000 container | `.jp2`, `.jpg2`       | `image/jp2`                         | Native Rust (hayro-jpeg2000) | Yes         | OCR for JP2 containers, table detection, format-specific metadata |
| JPEG 2000 codestream | `.j2c`, `.j2k`, `.jpc` | `image/j2c`                        | Native Rust (hayro-jpeg2000) | Yes         | OCR for raw JPEG 2000 codestreams, table detection               |
...
| HEIC / HEIF | `.heic`, `.heics`, `.heif`, `.heifs`, `.hif` | `image/heic`, `image/heic-sequence`, `image/heif`, `image/heif-sequence` | Native libheif binding | Yes | Pixel decoding requires `heic` and libheif; EXIF metadata is pure Rust |
...
| MP4 Video Audio | `.mp4`, `.mpg4`, `.mp4v`, `.m4v` | `video/mp4` | Whisper ONNX          | No          | Audio-track transcription only           |
| MPEG Video Audio | `.mpeg`, `.mpg`, `.mpe`, `.m1v`, `.m2v` | `video/mpeg` | Whisper ONNX       | No          | Audio-track transcription only           |
| WebM Video Audio | `.webm`         | `video/webm` | Whisper ONNX            | No          | Audio-track transcription only           |
| Windows Media | `.wmv`, `.asf` | `video/x-ms-wmv` | Whisper ONNX (audio via Windows Media Foundation) | No | Audio-track transcription only; an `ffmpeg` on `PATH` or `XBERG_FFMPEG` is used where the media codecs are absent |
...
| Typst            | `.typst`, `.typ`   | `text/vnd.typst`                                 | Native (typst-syntax)                                                                         | No          | Modern typesetting format                                                      |
...
| DocBook          | `.docbook`, `.dbk`, `.docbook4`, `.docbook5` | `application/docbook+xml`       | Native (roxmltree)                                                                            | No          | Technical documentation format                                                 |
| JATS             | `.jats`, `.nxml`   | `application/x-jats+xml`                         | Native (roxmltree)                                                                            | No          | Journal article XML format                                                     |
...
| ------------ | ------- |
| `pdf`        | PDF extraction through xberg-native-pdf, PDF images, PDF rendering helpers |
...
xberg = { version = "1", features = ["pdf", "excel"] }
```
...
```toml title="Cargo.toml"
[dependencies]
xberg = { version = "1", features = ["server"] }
```
...
3. **Manual Override**: A specific explicit MIME type is authoritative. `application/octet-stream`
   is a generic placeholder that triggers policy-based detection.
