...
<!-- alef:hash:42e9a189591ba0a047868123bb27335acede45b715ba64b884badf97dbda8a39 -->
<!-- To regenerate: alef docs -->
<!-- To verify freshness: alef verify -->

## Python API Reference <span class="version-badge">v1.1.0</span>
...
| `max_archive_depth` | `int` | `xberg::ExtractionConfig::default_archive_depth()` | Maximum recursion depth for archive extraction (default: 3). Set to 0 to disable recursive extraction (legacy behavior). |
...
| `class_prefix` | `str` | `default_class_prefix()` | CSS class prefix applied to every emitted class name. Default: `"kb-"`. Change this if your host application already uses classes that start with `kb-`. |
...
| `batch_size` | `int` | `default_batch_size()` | Batch size for local ONNX inference. ColBERT emits a `[seq, dim]` multi-vector embedding per document, so memory scales with batch size — keep this modest. |
| `max_length` | `int` | `default_max_length()` | Maximum token sequence length for the tokenizer (documents). |
| `query_max_length` | `int` | `default_query_max_length()` | Fixed padded length for query augmentation. ColBERT queries are padded (with the mask token, kept attention-live) to exactly this many tokens rather than truncated/left as-is — this is the "query augmentation" trick from the ColBERT paper. |
...
| `max_embed_duration_secs` | `int \| None` | `default_max_embed_duration_secs()` | Maximum wall-clock duration (in seconds) for a single embed call when using `LateInteractionModelType.Plugin`. `None` disables the timeout. |
...
| `backend` | `str` | `default_tesseract_backend()` | OCR backend: tesseract, paddleocr, paddle-ocr, sceptre, or vlm. Sceptre uses ONNX Runtime on desktop/server and tract on supported mobile builds. Browser WebAssembly uses the separate byte-fed Sceptre worker API. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `host` | `str` | `default_host()` | Server host address (e.g., "127.0.0.1", "0.0.0.0") |
...
| `max_request_body_bytes` | `int` | `default_max_request_body_bytes()` | Maximum size of request body in bytes (default: 100 MB) |
| `max_multipart_field_bytes` | `int` | `default_max_multipart_field_bytes()` | Maximum size of multipart fields in bytes (default: 100 MB) |
...
| `batch_size` | `int` | `default_batch_size()` | Batch size for local ONNX inference. SPLADE emits a `[seq, vocab]` logit tensor per document, so memory scales with batch size — keep this modest. |
| `max_length` | `int` | `default_max_length()` | Maximum token sequence length for the tokenizer. |
...
| `max_embed_duration_secs` | `int \| None` | `default_max_embed_duration_secs()` | Maximum wall-clock duration (in seconds) for a single embed call when using `SparseEmbeddingModelType.Plugin`. `None` disables the timeout. |
...
| `scheme_allowlist` | `list\[str\]` | `default_scheme_allowlist()` | Allowed URI schemes. Default: `["http", "https"]`. Only `http` and `https` are supported. An empty list denies every URL. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `mode` | `str` | `default_reduction_mode()` | Reduction mode: "off", "light", "moderate", "aggressive", "maximum" |
...
| `max_duration_ms` | `int \| None` | `default_max_duration_ms()` | Hard safety limit on input duration (milliseconds). Files longer than this are rejected after decode, before model work. Default: 30 minutes. Set to `None` to disable (not recommended for untrusted input). |
| `max_bytes` | `int \| None` | `default_max_bytes()` | Hard safety limit on input size (bytes). Default: 512 MiB. Protects against pathological or malicious uploads. |
| `timeout_ms` | `int \| None` | `default_timeout_ms()` | Wall-clock timeout for the entire transcription operation (ms). Bounds audio decode, model resolution/download, and inference together. On expiry the extraction fails with an `XbergError.Transcription`. `None` disables the bound and lets the operation run unbounded (not recommended for untrusted input). Enforced on the async extraction path only; the size and duration caps (`max_bytes`, `max_duration_ms`) are checked on every path. Default: 10 minutes. |
