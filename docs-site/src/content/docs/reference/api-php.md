...
<!-- alef:hash:f8cecb186671fc4941d1f366a54b981bf7dfb82cbf480855c285ad73b18201d1 -->
<!-- To regenerate: alef docs -->
<!-- To verify freshness: alef verify -->

## PHP API Reference <span class="version-badge">v1.1.0</span>
...
| `maxArchiveDepth` | `int` | `xberg::ExtractionConfig::default_archive_depth()` | Maximum recursion depth for archive extraction (default: 3). Set to 0 to disable recursive extraction (legacy behavior). |
...
| `batchSize` | `int` | `default_batch_size()` | Batch size for local ONNX inference. ColBERT emits a `[seq, dim]` multi-vector embedding per document, so memory scales with batch size — keep this modest. |
| `maxLength` | `int` | `default_max_length()` | Maximum token sequence length for the tokenizer (documents). |
| `queryMaxLength` | `int` | `default_query_max_length()` | Fixed padded length for query augmentation. ColBERT queries are padded (with the mask token, kept attention-live) to exactly this many tokens rather than truncated/left as-is — this is the "query augmentation" trick from the ColBERT paper. |
...
| `maxEmbedDurationSecs` | `?int` | `default_max_embed_duration_secs()` | Maximum wall-clock duration (in seconds) for a single embed call when using `LateInteractionModelType::Plugin`. `null` disables the timeout. |
...
| `port` | `int` | `default_port()` | Server port number |
...
| `maxRequestBodyBytes` | `int` | `default_max_request_body_bytes()` | Maximum size of request body in bytes (default: 100 MB) |
| `maxMultipartFieldBytes` | `int` | `default_max_multipart_field_bytes()` | Maximum size of multipart fields in bytes (default: 100 MB) |
...
| `batchSize` | `int` | `default_batch_size()` | Batch size for local ONNX inference. SPLADE emits a `[seq, vocab]` logit tensor per document, so memory scales with batch size — keep this modest. |
| `maxLength` | `int` | `default_max_length()` | Maximum token sequence length for the tokenizer. |
...
| `maxEmbedDurationSecs` | `?int` | `default_max_embed_duration_secs()` | Maximum wall-clock duration (in seconds) for a single embed call when using `SparseEmbeddingModelType::Plugin`. `null` disables the timeout. |
...
| `schemeAllowlist` | `array<string>` | `default_scheme_allowlist()` | Allowed URI schemes. Default: `["http", "https"]`. Only `http` and `https` are supported. An empty list denies every URL. |
...
| `maxDurationMs` | `?int` | `default_max_duration_ms()` | Hard safety limit on input duration (milliseconds). Files longer than this are rejected after decode, before model work. Default: 30 minutes. Set to `null` to disable (not recommended for untrusted input). |
| `maxBytes` | `?int` | `default_max_bytes()` | Hard safety limit on input size (bytes). Default: 512 MiB. Protects against pathological or malicious uploads. |
| `timeoutMs` | `?int` | `default_timeout_ms()` | Wall-clock timeout for the entire transcription operation (ms). Bounds audio decode, model resolution/download, and inference together. On expiry the extraction fails with an `XbergError::Transcription`. `null` disables the bound and lets the operation run unbounded (not recommended for untrusted input). Enforced on the async extraction path only; the size and duration caps (`max_bytes`, `max_duration_ms`) are checked on every path. Default: 10 minutes. |
...
| `OnLowQuality` | Try the classical OCR backend first. If the quality score is below `quality_threshold`, send the page to the VLM. `quality_threshold` is in the `[0.0, 1.0]` range produced by `text::quality::calculate_quality_score`. A value of `0.5` is a reasonable starting point; calibrate with the Stage 0 benchmark harness. — Fields: `qualityThreshold`: `float` |
