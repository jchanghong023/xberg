...
<!-- alef:hash:30ea31578a05e815d432eeefba4939be83656496f70d10e2b46be97009f8da8d -->
<!-- To regenerate: alef docs -->
<!-- To verify freshness: alef verify -->

## Java API Reference <span class="version-badge">v1.1.0</span>
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `model` | `EmbeddingModelType` | `default_balanced_embedding_model()` | The embedding model to use (defaults to "gte-modernbert-base" preset if not specified) |
...
| `maxArchiveDepth` | `long` | `xberg::ExtractionConfig::default_archive_depth()` | Maximum recursion depth for archive extraction (default: 3). Set to 0 to disable recursive extraction (legacy behavior). |
...
| `classPrefix` | `String` | `default_class_prefix()` | CSS class prefix applied to every emitted class name. Default: `"kb-"`. Change this if your host application already uses classes that start with `kb-`. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `model` | `LateInteractionModelType` | `default_late_interaction_model()` | The late-interaction model to use (defaults to the "gte-moderncolbert" preset). |
| `batchSize` | `long` | `default_batch_size()` | Batch size for local ONNX inference. ColBERT emits a `[seq, dim]` multi-vector embedding per document, so memory scales with batch size — keep this modest. |
| `maxLength` | `long` | `default_max_length()` | Maximum token sequence length for the tokenizer (documents). |
| `queryMaxLength` | `long` | `default_query_max_length()` | Fixed padded length for query augmentation. ColBERT queries are padded (with the mask token, kept attention-live) to exactly this many tokens rather than truncated/left as-is — this is the "query augmentation" trick from the ColBERT paper. |
...
| `maxEmbedDurationSecs` | `@Nullable Long` | `default_max_embed_duration_secs()` | Maximum wall-clock duration (in seconds) for a single embed call when using `LateInteractionModelType.Plugin`. `null` disables the timeout. |
...
| `backend` | `String` | `default_tesseract_backend()` | OCR backend: tesseract, paddleocr, paddle-ocr, sceptre, or vlm. Sceptre uses ONNX Runtime on desktop/server and tract on supported mobile builds. Browser WebAssembly uses the separate byte-fed Sceptre worker API. |
...
| `topMarginFraction` | `@Nullable Float` | `null` | Top margin fraction (0.0–1.0) of page height to exclude headers/running heads. Ignored when `ContentFilterConfig.include_headers` is `true`. Effective nonzero margins require per-page OCR so geometry can be filtered; document-capable OCR backends use their image-processing path in that case. Default: 0.06 (6%) |
| `bottomMarginFraction` | `@Nullable Float` | `null` | Bottom margin fraction (0.0–1.0) of page height to exclude footers/page numbers. Ignored when `ContentFilterConfig.include_footers` is `true`. Effective nonzero margins require per-page OCR so geometry can be filtered; document-capable OCR backends use their image-processing path in that case. Default: 0.05 (5%) |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `model` | `RerankerModelType` | `default_reranker_model()` | The reranker model to use (defaults to "balanced" preset if not specified). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `host` | `String` | `default_host()` | Server host address (e.g., "127.0.0.1", "0.0.0.0") |
| `port` | `short` | `default_port()` | Server port number |
...
| `maxRequestBodyBytes` | `long` | `default_max_request_body_bytes()` | Maximum size of request body in bytes (default: 100 MB) |
| `maxMultipartFieldBytes` | `long` | `default_max_multipart_field_bytes()` | Maximum size of multipart fields in bytes (default: 100 MB) |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `model` | `SparseEmbeddingModelType` | `default_sparse_model()` | The sparse-embedding model to use (defaults to the "opensearch-v3-distill" preset). |
| `batchSize` | `long` | `default_batch_size()` | Batch size for local ONNX inference. SPLADE emits a `[seq, vocab]` logit tensor per document, so memory scales with batch size — keep this modest. |
| `maxLength` | `long` | `default_max_length()` | Maximum token sequence length for the tokenizer. |
...
| `maxEmbedDurationSecs` | `@Nullable Long` | `default_max_embed_duration_secs()` | Maximum wall-clock duration (in seconds) for a single embed call when using `SparseEmbeddingModelType.Plugin`. `null` disables the timeout. |
...
| `schemeAllowlist` | `List<String>` | `default_scheme_allowlist()` | Allowed URI schemes. Default: `["http", "https"]`. Only `http` and `https` are supported. An empty list denies every URL. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `mode` | `String` | `default_reduction_mode()` | Reduction mode: "off", "light", "moderate", "aggressive", "maximum" |
...
| `maxDurationMs` | `@Nullable Long` | `default_max_duration_ms()` | Hard safety limit on input duration (milliseconds). Files longer than this are rejected after decode, before model work. Default: 30 minutes. Set to `null` to disable (not recommended for untrusted input). |
| `maxBytes` | `@Nullable Long` | `default_max_bytes()` | Hard safety limit on input size (bytes). Default: 512 MiB. Protects against pathological or malicious uploads. |
| `timeoutMs` | `@Nullable Long` | `default_timeout_ms()` | Wall-clock timeout for the entire transcription operation (ms). Bounds audio decode, model resolution/download, and inference together. On expiry the extraction fails with an `XbergError.Transcription`. `null` disables the bound and lets the operation run unbounded (not recommended for untrusted input). Enforced on the async extraction path only; the size and duration caps (`max_bytes`, `max_duration_ms`) are checked on every path. Default: 10 minutes. |
...
| `ON_LOW_QUALITY` | Try the classical OCR backend first. If the quality score is below `quality_threshold`, send the page to the VLM. `quality_threshold` is in the `[0.0, 1.0]` range produced by `text.quality.calculate_quality_score`. A value of `0.5` is a reasonable starting point; calibrate with the Stage 0 benchmark harness. — Fields: `qualityThreshold`: `double` |
