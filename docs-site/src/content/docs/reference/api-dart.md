...
<!-- alef:hash:7993fac31e5c47fa24488f1f133e5cc0fe3c30f3515a010a30cf73847846922b -->
<!-- To regenerate: alef docs -->
<!-- To verify freshness: alef verify -->

## Dart API Reference <span class="version-badge">v1.1.0</span>
...
| `maxArchiveDepth` | `PlatformInt64` | `xberg::ExtractionConfig::default_archive_depth()` | Maximum recursion depth for archive extraction (default: 3). Set to 0 to disable recursive extraction (legacy behavior). |
...
| `batchSize` | `PlatformInt64` | `default_batch_size()` | Batch size for local ONNX inference. ColBERT emits a `[seq, dim]` multi-vector embedding per document, so memory scales with batch size — keep this modest. |
| `maxLength` | `PlatformInt64` | `default_max_length()` | Maximum token sequence length for the tokenizer (documents). |
| `queryMaxLength` | `PlatformInt64` | `default_query_max_length()` | Fixed padded length for query augmentation. ColBERT queries are padded (with the mask token, kept attention-live) to exactly this many tokens rather than truncated/left as-is — this is the "query augmentation" trick from the ColBERT paper. |
...
| `maxRequestBodyBytes` | `PlatformInt64` | `default_max_request_body_bytes()` | Maximum size of request body in bytes (default: 100 MB) |
| `maxMultipartFieldBytes` | `PlatformInt64` | `default_max_multipart_field_bytes()` | Maximum size of multipart fields in bytes (default: 100 MB) |
...
| `batchSize` | `PlatformInt64` | `default_batch_size()` | Batch size for local ONNX inference. SPLADE emits a `[seq, vocab]` logit tensor per document, so memory scales with batch size — keep this modest. |
| `maxLength` | `PlatformInt64` | `default_max_length()` | Maximum token sequence length for the tokenizer. |
