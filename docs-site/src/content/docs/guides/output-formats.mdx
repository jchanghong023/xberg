...
- **Content Format** — `output_format` controls the `content` field: Plain, Markdown, Djot, HTML, JSON, [DocTags](/guides/doctags-output/), or a registered custom renderer such as [DOT](/guides/diagram-dot-output/)
...
detections are layered on top. Layout detection is off by default, and `Markdown` is the default
...
| ----------------- | ----------------------------------------------------------------------- |
| `Plain`          | Raw extracted text with minimal formatting                              |
| `Markdown` (default) | Markdown-formatted content                                          |
| `Djot`            | Djot markup                                                            |
| `Html`            | HTML-formatted content                                                  |
| `Json`            | JSON tree with heading-driven sections                                  |
| `DocTags`         | Docling DocTags tag-stream format (tables as OTSL). See [DocTags Output](/guides/doctags-output/) |
| `dot`             | Graphviz DOT for a diagram recovered from a vector SVG or PDF source; empty string when no diagram was recovered. See [Diagram DOT Output](/guides/diagram-dot-output/) |
...
| `Svg` | — | Archival of vector images | Lossless vector output. Raster sources return a warning; not auto-vectorized. Requires `svg` feature. |
...
<ApiSnippetGroup topic="contract/config_element_types" />
...
<ApiSnippetGroup topic="contract/config_document_structure_with_headings" />
...
| `k_clusters`   | `int`  | `3`     | Font size clusters (1–7), maps to heading levels    |
...
- **`hierarchy` is `None`** — Check `hierarchy.enabled` is `True`. This field is only populated by the
  native PDF text-tree pipeline; it stays `None` for OCR-backed (image-only) pages regardless of
  whether OCR is enabled — enabling OCR does not populate `hierarchy`. If fewer text blocks than
  `k_clusters`, reduce `k_clusters`. For structured output from a scanned PDF, use `output_format`
  (e.g. `markdown`) instead of relying on `hierarchy`.
