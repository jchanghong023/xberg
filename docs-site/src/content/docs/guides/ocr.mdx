...
| --- | --- | --- | --- |
| **Tesseract** | Broad language and platform support; low runtime overhead; hierarchical hOCR output | Requires a system package and separately installed language data; less reliable on scene text and difficult scans | You need the Tesseract fallback (builds without `paddle-ocr`), WebAssembly support, or a small deployment footprint |
...
:::note[No automatic classical fallback]
The default `backend` is already the strongest classical engine compiled in (`paddle-ocr` where the `paddle-ocr` feature is on, `tesseract` otherwise), so xberg does not mix classical backends on its own — a `[tesseract, paddleocr]` chain requires an explicit `pipeline`. Only `vlm_fallback` synthesises stages: the configured backend at priority 100, then `vlm` at 50 for `OnLowQuality`.
:::
