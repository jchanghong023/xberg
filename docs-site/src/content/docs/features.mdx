...
![Xberg features overview -- 110 input formats flow through extraction, OCR, and processing to produce text, tables, chunks, and metadata](../../assets/feature-overview.png)

---

## Format Support

110 file formats across 146 unique file extensions, with 58 compatibility MIME aliases, are handled by native Rust
extractors — no LibreOffice or other external tools required.
...
<span class="format-chip">PowerPoint <code>.pptx .ppt .pps</code></span>
...
<span class="format-chip">Djot <code>.djot .dj</code></span>
...
<span class="format-chip">Visio <code>.vsd .vsdx .vsdm</code></span>
</div>
...
<div class="format-chips">
<span class="format-chip">Excel <code>.xlsx .xls .xlsm .xlsb .xltm</code></span>
...
<span class="format-chip">JPEG 2000 <code>.jp2 .jpg2 .j2c .j2k .jpc</code></span>
...
<span class="format-chip">HEIF <code>.heif .heifs .hif</code></span>
...
<span class="format-chip">MP4 audio track <code>.mp4 .mpg4 .mp4v .m4v</code></span>
<span class="format-chip">MPEG audio track <code>.mpeg .mpg .mpe .m1v .m2v</code></span>
<span class="format-chip">WebM audio track <code>.webm</code></span>
<span class="format-chip">Windows Media <code>.wmv .asf</code></span>
</div>
...
<span class="format-chip">XHTML <code>.xhtml .xht</code></span>
...
<span class="format-chip">GeoJSON <code>.geojson</code></span>
<span class="format-chip">KML <code>.kml</code></span>
<span class="format-chip">YAML <code>.yaml</code></span>
<span class="format-chip">TOML <code>.toml</code></span>
<span class="format-chip">SQLite <code>.sqlite .sqlite3 .db</code></span>
<span class="format-chip">GeoPackage <code>.gpkg .gpkx</code></span>
</div>
...
<span class="format-chip">CSL JSON <code>MIME only</code></span>
<span class="format-chip">LaTeX <code>.tex</code></span>
<span class="format-chip">Typst <code>.typ .typst</code></span>
...
1. **MIME detection** -- Xberg prefers bounded content inspection, then falls back to a supported filename extension.
   Unknown or missing extensions are sniffed rather than rejected. Configure `mime_detection_policy` to trust a
   supported extension or ignore extensions when your input boundary requires different behavior.
...
- Model size: 6.7 GB (BF16 weights) on first download, auto-downloaded from a checksum-pinned
  revision of `deepseek-ai/DeepSeek-OCR`; cached at `~/.cache/huggingface/` (or `HF_HUB_CACHE` /
  `HUGGINGFACE_HUB_CACHE` / `HF_HOME`) — no `backend_options.model_path` required
- Fine-grained layout detection, table region recognition, text extraction with confidence scores
- Pages wider or taller than 640 px are tiled into 640 px local-crop tiles (the reference "Gundam"
  mode) in addition to the 1024 px global view, so small text keeps its resolution
- Dtype auto-selects per device (override via `backend_options.dtype`): CPU F32, CUDA BF16, Metal F16
...
The table above covers the main entry points. These are lower-level or opt-in flags not otherwise
documented — each enables narrower functionality and carries a specific cost (extra native
dependency, platform restriction, or CI-untested status per Cargo.toml's own comments).
...
`sparse-embedding-presets`, `late-interaction-presets`, `api-types`, `onnx-runtime`,
