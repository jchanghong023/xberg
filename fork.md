...
**状态与核对约定**：本清单描述「同步上游后仍需在本 fork 成立的行为」，不是 Git 差异摘要或开发日志；对照基线 = `git merge-base main upstream/main`。下列条目由现行实现与本地提交恢复而来，验收 = `fulltest.py` 对应检查不回归 + 各条给出的可观察行为。同步上游后必须逐项复核（上游可能已独立实现或改变同一行为），不能只看有没有 Git 冲突。最近一次复核：2026-09-24 对 upstream/main（1.2.9，并入 51 提交，主体为 PDF XY-cut 列切分/段内拆分修复（GH#1763）、扫描页质量门与三个 OCR 行为显式设置（GH#1752：`ocr_embedded_images`/`ocr_near_empty_fallback`/`ocr_scanned_page_quality_gate`）、code-page 乱码路由 OCR、native-pdf 页树递归深度与内容流体积上限、lib.rs 巨型文件辅助函数抽取重构）逐项核对——冲突 21 文件：14 个版本号/依赖引用文件取上游 1.2.9 并保留 fork 110 formats 计数；7 个 Rust 文件中，`core/config/extraction/core.rs` 采纳上游 `ocr_embedded_images` 显式开关但默认改 fork 语义（`None` 时视为 `true`，无 `ocr` 块也 OCR 内嵌图片），`core/pipeline/mod.rs` 与 `extraction/image_ocr.rs` 采纳上游单一门 `runs_ocr_on_embedded_images()` + `automatic_ocr_backend_is_registered()` 结构（上游测试禁止内联副本；fork 的 disable_ocr 硬开关已并入该谓词、无 `ocr` 块回退默认后端的语义由 `OcrConfig::default()`＝paddle 承载；fork 原「后端未注册告警一次」由上游的自动触发跳过/逐图告警语义替代，仅影响误配场景），`core/pipeline/initialization/mod.rs` 的 registry 重试测试两边同修、取上游更精确版（只重试 REGISTRY_IN_USE 拒绝），`pdf/scan_detect.rs` 与 `pdf/native/text.rs` 两边独立做了 #1744 provenance 计数器 thread-local 化、统一取上游 `thread_local` + `AtomicUsize` 版，`crates/xberg/Cargo.toml` 保 fork 依赖块（windows-metafile、windows MF）并随上游 bump 1.2.9。清单全部条目仍成立：`effective_pipeline()` 默认 `None`、默认 OCR 后端 paddle-ocr、默认输出/图片抽取、perf span 契约、16 MiB 栈、profiles 与 110 formats 计数均验证在位（`cargo check` + fastcheck 通过；fulltest/slowtest 门随本次发布流程执行，结果见当次质量报告）。
