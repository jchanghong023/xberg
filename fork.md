...
**状态与核对约定**：本清单描述「同步上游后仍需在本 fork 成立的行为」，不是 Git 差异摘要或开发日志；对照基线 = `git merge-base main upstream/main`。下列条目由现行实现与本地提交恢复而来，验收 = `fulltest.py` 对应检查不回归 + 各条给出的可观察行为。同步上游后必须逐项复核（上游可能已独立实现或改变同一行为），不能只看有没有 Git 冲突。最近一次复核：2026-09-26 对 upstream/main（1.3.0，自 e2cf0865d5 再 62 提交至 d60b31b1a4：1.3.0 发布、GH#1789 数字修复（opt-in 默认 false）、tesseract thresholding_method 校验、#1831 后端查找编译期门控、OTel 0.33、上游「18 个超大文件测试模块外移」重构、第二批 12 提交的 PDF 表格/Type 3 字体/扫描页 OCR 路由修复）逐项核对——第一批冲突 18 文件：4 核心为 `crates/xberg/Cargo.toml`（保 fork 的 `xberg-windows-metafile`+`windows` 依赖，版本号统一 1.3.0）、`core/image_encode.rs`（采纳上游 `mod tests;` 外移结构，fork 的 21 个测试含 fork 特有 `native_target_still_routes_metafiles_to_the_rasterizer` 写入 `image_encode/tests.rs`；fork `re_encode` 5 参签名（`image_config`，metafile 链路）保留并把上游 tests.rs 调用适配回 5 参）、`extractors/image.rs`（上游 GH#1789 的 cfg 互斥 `ocr_content` 绑定与 fork 的 #1571 move 语义注释并存）、`pdf/native/images.rs`（采纳上游 tests/decode_skip_tests/parallel_tests 三文件结构；fork row-padded repack 行为保留，上游 3 个 `returns_error` 测试替换为 fork 的 3 个 `repacks_instead_of_panicking` 测试）；14 个 integrations/plugin 文件为版本号，取上游；第二批 12 提交无冲突，但上游自带的 `pdf/scan_detect/tests.rs` fmt 漂移（本机 rustfmt 1.9.0-stable 判定，`cargo fmt -p xberg` 重排 666 行缩进修复）。清单全部条目仍成立：默认输出/图片抽取/OCR 后端与 `ocr_embedded_images` 语义、`effective_pipeline()` 默认 `None`、16 MiB 栈、perf span 契约（OTel 0.33 升级不影响 `core-cli → otel` 恒开与三分支互斥）、profiles 与 feature 集不变、格式计数不变（上游本次未动 formats.rs）。验证：fastcheck PASS 5.8s；`cargo check`（fork 集）PASS 2m09s；fulltest.py / slowtest.py 未跑（未授权，转换质量待用户验证）。上一轮（同日，1.2.9 后 20 提交至 e2cf0865d5）：冲突 2 文件——`extractors/mod.rs` 采纳上游 register_baseline/office/container_format 三函数注册重构，fork 的 VisioExtractor 挪进 `register_office_extractors`、测试计数断言补 `visio-extractor`；`ocr/processor/config.rs` 双方测试并存，fork 的 security_limits 缓存键断言迁移到上游 #1787 双参数 `hash_config(config, resolved_tessdata)` 签名。当轮验证：fulltest.py FAIL=0 WARN=7、对抗全 PASS；slowtest.py 通过（FAIL=0 WARN=7，含 `--deep` 音视频段，343s）；对 9-15 基线 新增 2（第三课/第九课的 ENGINE_WARN，系 2026-09-19 Visio 内容路由修复让内嵌 VSD 如实报错的既有产物，非合并引入）/已修复 2（tessent `CAPTION_IN_TABLE`、测试识别.png `GOLDEN_METRIC`）/恶化 0。
