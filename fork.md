...
**状态与核对约定**：本清单描述「同步上游后仍需在本 fork 成立的行为」，不是 Git 差异摘要或开发日志；对照基线 = `git merge-base main upstream/main`。下列条目由现行实现与本地提交恢复而来，验收 = `fulltest.py` 对应检查不回归 + 各条给出的可观察行为。同步上游后必须逐项复核（上游可能已独立实现或改变同一行为），不能只看有没有 Git 冲突。最近一次复核：2026-09-23 对 upstream/main（`7a6f084a0e`，1.2.8，并入 77 提交，主体为超大文件拆分子模块的复杂度重构与 lint 基线清偿，功能面有 GH#1732 整页图像跳解码快路、GH#1744 provenance 免二次读、表格悬挂缩进与 PDF 混排修复、依赖升级 ureq/utoipa/tract-onnx/unhwp/jsonschema）逐项核对——冲突 25 文件：14 个版本号文件取上游 1.2.8 并保留 fork 110 formats 计数；上游把 `cli commands/{extract,overrides}`、`extraction/{excel,email,derive,ooxml_embedded}` 拆成子模块（UD 冲突），fork 定制全部移植到新位置——CLI `extract/images.rs` 的 `--output-dir` 百分号编码前缀 + 批处理 `doc_N` 目录 + perf span、`overrides` 的 paddle 默认后端测试与解锁的 tracing 辅助、`excel/open.rs` 的 xls/xla CFB zip 容错与空 used-range 行丢弃（含 `mod images` 声明与配套测试）、`email/msg.rs` 的 `\`↔`/` 路径归一化、`derive/mod.rs` 的 pre-rendered 兜底、`ooxml_embedded/` 的内嵌对象文本合并 + 图片引用重编号舰队与 OLE payload 识别（Ole10Native/Package/CompObj）及 3 参签名；UU 冲突取上游辅助函数结构、fork 行为内嵌（`image_ocr.rs` 保留 EMF/WMF 光栅化任务与禁用 OCR 守卫并采纳上游 spawn/apply 拆分、`markdown.rs` 注释过滤围栏感知进 helper、collapse/watermark 保 fork 包裹、`pptx` 采纳 elements 拆分并全局 `join_runs`、`html_styled.rs` alt 清洗进 `render_image_element`、`pdf/extraction.rs` 双方新函数并存）。上游 GH#1732「OCR 唯一消费者」快路与 fork 默认抽取图片并存（其测试按 fork 图片输出默认改构造）；#1744 测试计数器 thread_local 化以消除 libtest 并行竞态。清单全部条目仍成立：`effective_pipeline()` 默认 `None`、默认 OCR 后端 paddle-ocr（上游 1.2.8 已改用 `OcrConfig::default` 兜底，与 fork 语义一致）、默认输出/图片抽取、perf span 契约、16 MiB 栈、profiles 与 110 formats 计数均验证在位（`cargo check` + fulltest 门 12 阶段全绿，质量报告 FAIL=0/WARN=7，与基线比新增 2/已修复 1/恶化 0）。
