...
fn test_confidence_threshold_boundary_low() {
    let text = "This is a comprehensive English sentence with multiple words to ensure accurate language detection.";
...
fn test_russian_ukrainian_document() {
    let text = format!(
        "{}{}",
        "Россия является крупнейшей страной в мире по территории. Москва - столица России и крупнейший город страны. "
            .repeat(5),
...
fn test_special_characters_with_text() {
    let text = "The company's revenue increased by 25% year-over-year. CEO said: \"We're excited!\" #growth @investors";
    let config = LanguageDetectionConfig {
        enabled: true,
        min_confidence: 0.5,
        detect_multiple: false,
