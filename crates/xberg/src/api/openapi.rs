...
#[openapi(
    info(
        ...
        description = "High-performance document intelligence API for extracting text, metadata, and structured data from PDFs, Office documents, images, and 110 formats.",
        contact(
            name = "Xberg",
            url = "https://xberg.io"
        ),
...
mod tests {
    ...
    /// The concrete second-order nested types from #1424, named so a regression fails
    /// with a readable diff instead of a list of raw `$ref` strings. ~keep
    #[cfg(feature = "api")]
    const SECOND_ORDER_COMPONENTS_FROM_ISSUE_1424: &[&str] = &[
        "PageStructure",
        "PageUnitType",
        "PageBoundary",
        "PageInfo",
    ...
    fn should_define_second_order_nested_schemas_from_issue_1424() {
        ...
        let missing: Vec<&str> = SECOND_ORDER_COMPONENTS_FROM_ISSUE_1424
            .iter()
            .copied()
