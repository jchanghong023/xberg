...
mod tests {
    ...
    /// Build a single-member ZIP holding `payload` at `member`.
    fn build_zip_with_member(member: &str, payload: &[u8]) -> Vec<u8> {
        use std::io::Write;
    ...
    /// Rewrite `member`'s *declared* uncompressed size in both the local file header and the
    /// central directory, leaving the deflate stream and the CRC untouched. The member still
    /// reads back cleanly; only its size claim is a lie, which is what
    /// `IwaExpansionBudget::validate_member_size` -- reading the central directory -- cannot see.
    fn forge_declared_size(mut data: Vec<u8>, member: &str, forged_size: u32) -> Vec<u8> {
        ...
        while i + 46 <= data.len() {
            if data[i..i + 4] == *b"PK\x01\x02" {
                ...
                if i + 46 + name_len <= data.len() && data[i + 46..i + 46 + name_len] == *name {
                    let local_offset =
        ...
        assert_eq!(patched, 1, "exactly one central-directory record must be patched");
        data
    }

    /// Frame `payload` as a single uncompressed IWA block (type `0x01` + 3-byte LE length).
    fn uncompressed_iwa_block(payload: &[u8]) -> Vec<u8> {
        let len = payload.len();
        let mut framed = vec![
            0x01,
            (len & 0xFF) as u8,
    ...
    /// GHSA-85w9-wqcq-x48r: `validate_member_size` checks the archive's *declared* uncompressed
    /// size, and the `zip` crate puts no `Take` on the decompressed side, so `read_to_end` used
    /// to inflate a forged member without bound. A member declaring 32 bytes while carrying
    /// 1 MiB must be rejected, not read.
    ///
    /// Neutralisation that must break this test: restore `file.read_to_end(&mut raw)` in place
    /// of the `take(declared_size + 1)` bound in `read_iwa_file`.
    #[test]
    fn test_read_iwa_file_rejects_member_larger_than_its_declared_size() {
        let payload = uncompressed_iwa_block(&b"B".repeat(1024 * 1024));
        let zipped = forge_declared_size(
            build_zip_with_member("Index/Document.iwa", &payload),
            "Index/Document.iwa",
            32,
        );

        let mut expansion = IwaExpansionBudget::from_limits(&SecurityLimits::default());
        let result = read_iwa_file(&zipped, "Index/Document.iwa", &mut expansion);

        assert!(
            result.is_err(),
            "a member delivering more bytes than it declared must be rejected, got {} bytes",
            result.map(|r| r.len()).unwrap_or(0)
        );
    }

    /// Positive control for the bound above: a bound that rejected every member would also pass
    /// a test that only checks `is_err()`. An honestly-declared IWA member must still decode.
    #[test]
    fn test_read_iwa_file_accepts_member_with_honest_declared_size() {
        let text = b"Honest IWA payload";
        let payload = uncompressed_iwa_block(text);
        let zipped = build_zip_with_member("Index/Document.iwa", &payload);

        let mut expansion = IwaExpansionBudget::from_limits(&SecurityLimits::default());
        let decoded = read_iwa_file(&zipped, "Index/Document.iwa", &mut expansion)
            .expect("an honestly-declared IWA member must decode");

        assert_eq!(
            decoded, text,
            "the decoded stream must be the block payload, byte for byte"
        );
    }

    const TEST_PROPERTIES_PLIST: &str = r#"<?xml version="1.0" encoding="UTF-8"?>
<plist version="1.0"><dict>
  <key>title</key>
  <string>Forged Size Fixture</string>
</dict></plist>"#;

    /// The `Metadata/` members are read for every iWork document with no configuration toggle,
    /// and had no read bound at all. The bound is the smaller of the declared size and
    /// `MAX_METADATA_MEMBER_SIZE`, so a member forging a tiny declared size is truncated at that
    /// claim rather than inflated -- observable here as the title failing to parse out of the
    /// truncated prefix, where the same bytes with an honest declaration parse fine.
    ///
    /// Neutralisation that must break this test: restore the unbounded
    /// `file.read_to_end(&mut buf)` in `extract_metadata_from_zip`.
    #[test]
    fn test_metadata_plist_read_is_bounded_by_declared_member_size() {
        let honest = build_zip_with_member("Metadata/Properties.plist", TEST_PROPERTIES_PLIST.as_bytes());
        let forged = forge_declared_size(honest.clone(), "Metadata/Properties.plist", 8);

        assert_eq!(
            extract_metadata_from_zip(&honest).title.as_deref(),
            Some("Forged Size Fixture"),
            "an honestly-declared plist must be read in full and parsed"
        );
        assert_eq!(
            extract_metadata_from_zip(&forged).title,
            None,
            "a plist declaring 8 bytes must be truncated at that claim, not read in full"
        );
    }
