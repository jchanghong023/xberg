...
impl<'a> WpdWalker<'a> {
    ...
    fn walk(&mut self, events: &[WpdEvent]) {
        for event in events {
            ...
            match event {
                ...
                | WpdEvent::RowEnd
                | WpdEvent::TableEnd => {
                    unreachable!("table events are intercepted by handle_table_event before this match")
                }
            }
        }
    ...
    /// Handles the table-structure events (`TableStart`..`TableEnd`), returning `true` if
    /// `event` was one of them. Split out of [`Self::walk`] purely to shorten that method.
    fn handle_table_event(&mut self, event: &WpdEvent) -> bool {
        match event {
            WpdEvent::TableStart => {
