...
/// Validate the 18-byte `METAHEADER` that begins every standard (non-placeable) WMF.
///
/// The placeable wrapper's 4-byte magic key is only a weak hint, so structure is checked
/// here as well: word count, header size, version, declared size, and the maximum-record
/// bound all have to agree before treating the bytes as a playable metafile.
...
/// Validate the 88-byte `ENHMETAHEADER` that begins every EMF.
///
/// `" EMF"` at offset 40 plus record type 1 are the magic, but a truncated or forged
/// header must not be handed to GDI's `SetEnhMetaFileBits`, so the declared header size
/// and total byte count are bounded against the actual input here.
...
mod tests {
    ...
    #[test]
    fn test_detect_emf() {
        // Minimal but structurally valid `ENHMETAHEADER`: record type 1, 88-byte header,
        // `" EMF"` signature, and a total byte count that matches the input length.
        let mut data = vec![0u8; 88];
        data[0..4].copy_from_slice(&1u32.to_le_bytes());
        data[4..8].copy_from_slice(&88u32.to_le_bytes());
        data[40..44].copy_from_slice(b" EMF");
        data[48..52].copy_from_slice(&88u32.to_le_bytes());
        assert_eq!(detect_image_format(&data), "emf");
    }

    #[test]
    fn test_rejects_truncated_emf_header() {
        // The `" EMF"` magic alone is not enough: without a header that fits the input and a
        // total byte count bounded by it, the bytes must not be reported as an EMF.
