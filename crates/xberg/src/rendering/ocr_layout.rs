...
pub(crate) fn layout_boxes(items: &[(f64, f64, f64, String)]) -> Option<String> {
    ...
    let mut grid = rank_rows(&indexed, line_height).and_then(|rows| place_rows(&indexed, rows, column_px, min_left));
    if grid.is_none() {
        let rows = band_rows(&indexed);
        grid = place_rows(&indexed, rows, column_px, min_left);
        if grid.is_some() {
...
/// `indexed` order) and render it, or `None` when any line cannot be placed without reordering.
///
...
fn place_rows(
    ...
    let mut placed: Vec<(usize, usize, usize, &str, usize)> = indexed
        .iter()
        .zip(row_of_item)
        .map(|((index, left, _top, _height, text), row)| {
            ...
            (*index, row, column, text.as_str(), width)
        })
        .collect();
    ...
    for (_index, row, column, text, width) in placed {
        if occupied_rows >= MAX_ROWS {
            ...
            tracing::debug!(MAX_ROWS, "OCR grid refused: row cap reached");
            return None;
        }
        ...
        // the row cap above.
        if width > MAX_COLS {
            tracing::debug!(width, MAX_COLS, "OCR grid refused: a line is wider than the grid");
            return None;
        }
        ...
        loop {
            if row_index >= MAX_ROWS {
                ...
                tracing::debug!(MAX_ROWS, row_index, "OCR grid refused: a line lands past the row cap");
                return None;
            }
...
///
/// The grid's last resort: the placement pass refuses whenever a reconstruction would reorder a
/// row (see [`place_rows`]) or when a line cannot fit the grid at all, and the flat OCR text the
...
mod tests {
    ...
    /// A row whose left-to-right order disagrees with the OCR order refuses the grid (the
    /// reconstruction would invent a different reading order); the indented fallback must still
    /// carry every line, in the OCR's order, with each line's measured offset kept.
    #[test]
    fn indented_fallback_keeps_line_order_and_measured_columns() {
        let items = vec![
        ...
        assert!(
            ...
            "a same-row inversion must refuse the grid"
        );
