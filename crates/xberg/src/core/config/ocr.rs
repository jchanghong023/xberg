...
impl OcrConfig {
    ...
    pub(crate) fn effective_tesseract_language(&self) -> Vec<String> {
        ...
        resolved.iter().map(|code| tesseract_language_name(code)).collect()
    }
...
///
/// Lives in the config layer because [`OcrConfig::effective_tesseract_language`]
/// compiles under wider feature combinations than `feature = "ocr"` alone
/// (`ocr-pipeline`, `paddle_ocr`, `liter-llm`), so the `ocr`-gated modules it used
/// to sit beside are not always compiled. Pure mapping, no I/O.
///
...
mod tests {
    ...
    fn expected_default_backend() -> &'static str {
        if cfg!(paddle_ocr) { "paddle-ocr" } else { "tesseract" }
    }
