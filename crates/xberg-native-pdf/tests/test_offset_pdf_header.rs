//! Regression tests for Linearized PDFs whose `%PDF-` header is offset from
//! byte 0 by leading garbage, combined with sparse `/Root`-less trailers.
//!
...
/// The full scenario: leading garbage AND a sparse final trailer that
/// omits `/Root`. Before the fix this failed with
/// "Invalid PDF: Trailer missing /Root entry".
#[test]
fn garbage_prefix_with_sparse_rootless_trailer() {
    ...
    let pages = doc
        .page_count()
        .unwrap_or_else(|e| panic!("page_count failed for garbage-prefixed PDF: {e}"));
...
#[test]
fn garbage_prefix_with_root_bearing_trailer_still_works() {
    let mut bytes = html_redirect_garbage();
...
/// exactly like the real `medium.pdf`: `<</Size 114/ID[...]>>`).
...
/// The discriminating case: two trailers, the *later* one sparse and
...
#[test]
fn linearized_two_trailers_keeps_root_bearing_trailer() {
    let mut bytes = html_redirect_garbage();
