...
impl<'doc> TextExtractor<'doc> {
    ...
    ///
    /// ```
    /// use xberg_native_pdf::extractors::{TextExtractor, TextExtractionConfig};
    ///
    ...
    pub fn with_config(config: TextExtractionConfig) -> Self {
        ...
        Self {
            ...
            tj_offset_history: Vec::with_capacity(1000),
            tj_sum: 0.0,
            tj_sum_sq: 0.0,
            tj_stats_len: 0,
            tj_character_array: Vec::new(),
    ...
    pub fn set_page_index(&mut self, page_index: u32) {
        ...
        // finishes); update it in place. ~keep
    ...
    ///
    /// ```
    /// use xberg_native_pdf::extractors::{TextExtractor, SpanMergingConfig};
    ///
    ...
    fn sort_spans_by_reading_order(&mut self) {
        ...
        if columns.len() <= 1 {
            tracing::debug!("Using simple Y-then-X sorting (single column)");
            self.simple_sort_spans();
        } else {
            tracing::debug!("Using column-aware sorting ({} columns)", columns.len());
    ...
    fn detect_span_columns(&self) -> Vec<(f32, f32)> {
        ...
        tracing::debug!("Detected {} columns: {:?}", columns.len(), columns);
    ...
    fn dedup_stroke_fill_overlap(&mut self) {
        ...
        if skipped > 0 {
            tracing::debug!("Stroke+fill dedup: dropped {skipped} duplicate spans of {old_len}");
        }
        self.spans = kept;
    }
    ...
    fn detect_rtl_draw_direction(&mut self) {
    ...
    fn merge_adjacent_spans(&mut self) {
        ...
        let ink_boxes: Vec<Rect> = spans
            ...
            .map(|s| s.bbox)
            .collect();
        // Per-line bimodal word-gap thresholds, indexed to `spans`,
        // used below to rescue a narrow word gap the fixed kerning guard
        // suppressed. Computed before the fold consumes the list. ~keep
        ...
        for (span_idx, span) in spans.into_iter().enumerate() {
            ...
            // Safety: checked is_none() above which continues, so this is always Some. ~keep
            ...
            // the wmode metadata for the vertical glyph. ~keep
            ...
            // in the opposite X direction. ~keep
            let rotation_compatible = current.rotation_degrees == 0.0 && span.rotation_degrees == 0.0;
            ...
            // a space is warranted. ~keep
            let reliable_widths = self
                .fonts
                .get(&current.font_name)
                .map(|f| f.has_explicit_widths())
                .unwrap_or(true);
            let space_gap = corrected_space_gap(gap, reliable_widths, current.bbox.width, current.text.is_empty());
            ...
            // These should always be merged WITH a space, never without. ~keep
            ...
            // (e.g. "60000≤Q＜80000"). Treating them as a CJK
            // boundary would split the compound token. ~keep
            let is_cjk_char = |c: char| {
                matches!(
                    c as u32,
                    0x3040..=0x309F      // Hiragana ~keep
                    | 0x30A0..=0x30FF    // Katakana ~keep
                    | 0x3400..=0x4DBF    // CJK Unified Ideographs Extension A ~keep
                    | 0x4E00..=0x9FFF    // CJK Unified Ideographs ~keep
                    | 0xAC00..=0xD7AF    // Hangul Syllables ~keep
                    | 0x20000..=0x2A6DF  // CJK Unified Ideographs Extension B ~keep
                    | 0xFF66..=0xFF9F    // Halfwidth Katakana ~keep
                )
            };
            ...
            // makes such a change a word boundary. ~keep
            ...
            // Same-font spans are merged more aggressively to reconstruct words. ~keep
            let merge_threshold_pt = if is_same_font {
                ...
                // to handle minor kerning/rounding issues, but generally keep separate. ~keep
                0.5
            };
            ...
            let should_merge = (same_line
                && is_same_font
                && same_mcid
                && (self.merging_config.severe_overlap_threshold_pt..merge_threshold_pt).contains(&gap)
            ...
            //
            // Without a minimum-gap floor this
            ...
            // mantissa), and fusing those fabricates a decimal. ~keep
            ...
            // integer/cents boxes from widely-spaced subscripts. ~keep
            ...
            // `current.text` / `current.bbox`.) ~keep
            ...
            if decimal_merge {
                ...
                // or space-heuristic — these are same-word character runs. ~keep
                current.text.push_str(&span.text);
            } else if should_merge {
                // Check if next span is entirely whitespace-only OR marked as offset_semantic space
                // If either is true, never insert an additional space - just concatenate directly
                // This prevents double-space issue when TJ processor creates space spans ~keep
                ...
                if next_is_whitespace_only {
                    ...
                    // Narrow-word-gap rescue. The fixed intra-word
                    ...
                    //     empty. (`λᵢ` must not become `λ i`.) ~keep
                    let same_baseline =
                        (current.bbox.y - span.bbox.y).abs() < current.font_size.max(span.font_size).max(1.0) * 0.04;
                        ...
                        && !gap_has_intervening_glyph(&ink_boxes, &current.bbox, &span.bbox)
                    {
                        ...
                        // instances whose gap distribution is not cleanly bimodal. ~keep
                        if let Some(thr) = line_thresholds.get(span_idx).copied().flatten()
                            && gap > thr
                        {
                            space_decision = SpaceDecision::insert(SpaceSource::GeometricGap, 0.9);
                        }
                    }
                    ...
                    // geometric-spaced Arabic producers are unaffected. ~keep
                    if self.saw_reversed_chars && space_decision.insert_space {
                        ...
                        let next_ar = span.text.chars().next().is_some_and(|c| is_arabic_letter(c as u32));
                        if prev_ar && next_ar {
                    ...
                    if space_decision.insert_space {
                        ...
                        if next_is_offset_semantic_space {
                            tracing::trace!("Suppressing space insertion: next span is already TJ-offset space");
                            current.text.push_str(&span.text);
                        } else {
                            let would_create_double_space = current.text.ends_with(' ') && span.text.starts_with(' ');
                            ...
                            if would_create_double_space {
                                ...
                                tracing::trace!("Space via {:?}", space_decision.source);
                                current.text.push(' ');
                                current.text.push_str(&span.text);
                            }
                        }
                    } else {
                        tracing::trace!("No space insertion: decision source={:?}", space_decision.source);
                        current.text.push_str(&span.text);
                    }
                }
            }
            ...
            if decimal_merge || should_merge || cross_font_word_glue {
                ...
                // drawn right-to-left (see `detect_rtl_draw_direction`). ~keep
                ...
                // contribution at its own position instead. ~keep
                ...
                //    a partially-populated one keeps the legacy tail-pad. ~keep
                ...
                //    fallback for overlapping/degenerate layouts. ~keep
                let merged_char_count = current.text.chars().count();
                let separator_count = merged_char_count.saturating_sub(current_chars_before + span_char_count);
                ...
                // Latin/CJK output stays byte-identical. ~keep
                    ...
                    && (touches_arabic(&current.text) || touches_arabic(&span.text))
                {
                    ...
                    // space-inserted) merged text, and its target relative x. ~keep
        ...
        tracing::debug!("Merged adjacent spans: {} -> {} spans", old_len, merged.len());
    ...
    fn sort_by_reading_order(&mut self) {
        self.chars.sort_by(|a, b| {
            ...
            // Round Y coordinates to ensure transitivity of the comparison function ~keep
            let a_y_rounded = a.bbox.y.round() as i32;
            let b_y_rounded = b.bbox.y.round() as i32;
    ...
    fn execute_operator(&mut self, op: Operator) -> Result<()> {
        match op {
            ...
            Operator::Tj { text } => {
                ...
                if let Some(actual_text) = current_at {
                    if already_emitted {
                        ...
                        tracing::trace!("Tj operator: emitting MC-scope ActualText '{}'", actual_text);
                        self.mark_actual_text_emitted();
            ...
            Operator::SetFillColorSpace { name } => {
                ...
                if ink_excluded {
                    tracing::trace!("Fill color space {:?} matches excluded ink, suppressing text", name);
                }
            ...
            Operator::SetFillColor { components } => {
                ...
                match state.fill_color_space.as_str() {
                    ...
                    "ICCBased" => {
                        ...
                        tracing::trace!("ICCBased color space using simplified conversion (ICC profile not processed)");
                    }
                    "Separation" if components.len() == 1 => {
                        ...
                        state.fill_color_rgb = (gray, gray, gray);
                        tracing::trace!("Separation color space simplified to grayscale");
                    }
                    "DeviceN" if !components.is_empty() => {
                        ...
                        tracing::trace!("DeviceN color space using simplified conversion");
                    }
                    _ => {
            ...
            Operator::SetStrokeColor { components } => {
                ...
                match state.stroke_color_space.as_str() {
                    ...
                    "Lab" if components.len() == 3 => {
                        ...
                        tracing::trace!("Lab stroke color space simplified to grayscale");
                    }
                    "DeviceCMYK" if components.len() == 4 => {
                    ...
                    "ICCBased" => {
                        ...
                        tracing::trace!("ICCBased stroke color using simplified conversion");
                    }
                    "Separation" if components.len() == 1 => {
                        let tint = components[0];
                        let gray = 1.0 - tint;
                        state.stroke_color_rgb = (gray, gray, gray);
                        tracing::trace!("Separation stroke color simplified to grayscale");
                    }
                    "DeviceN" if !components.is_empty() => {
                        if components.len() == 4 {
                            state.stroke_color_cmyk =
                        ...
                        tracing::trace!("DeviceN stroke color using simplified conversion");
                    }
                    _ => {
                        match components.len() {
                            1 => {
            ...
            Operator::SetFillColorN { components, name } => {
                if name.is_some() {
                    ...
                    tracing::trace!("Pattern fill color not yet supported: {:?}", name);
                } else {
                    let state = self.state_stack.current_mut();
                    match state.fill_color_space.as_str() {
                        "DeviceGray" | "CalGray" if components.len() == 1 => {
            ...
            Operator::SetStrokeColorN { components, name } => {
                if name.is_some() {
                    // Pattern color space - for now, just log and ignore ~keep
                    tracing::trace!("Pattern stroke color not yet supported: {:?}", name);
                } else {
                    let state = self.state_stack.current_mut();
                    match state.stroke_color_space.as_str() {
                        "DeviceGray" | "CalGray" if components.len() == 1 => {
            ...
            Operator::BeginMarkedContent { tag } => {
                ...
                if is_artifact {
                    tracing::trace!("Entered /Artifact marked content (BMC, no subtype)");
                }
            }
            ...
            Operator::BeginMarkedContentDict { tag, properties } => {
                ...
                if let Some(props_dict) = self.resolve_bdc_properties(&properties) {
                    if let Some(mcid_obj) = props_dict.get("MCID") {
                        ...
                        if let Some(mcid) = mcid_obj.as_integer().and_then(crate::structure::checked_mcid) {
                            ...
                            tracing::trace!("Entered marked content with MCID: {}", mcid);
                        }
                    }
                        ...
                        && let Some(text_bytes) = actual_text_obj.as_string()
                    {
                        ...
                        tracing::trace!("Marked content has ActualText: {:?}", actual_text);
                        ...
                        && let Some(text_bytes) = expansion_obj.as_string()
                    {
                        ...
                        tracing::trace!("Marked content has expansion /E: {:?}", expansion);
                    }
                ...
                if is_artifact {
                    if let Some(ref atype) = artifact_type {
                        tracing::trace!("Entered /Artifact marked content: {:?}", atype);
                    } else {
                        tracing::trace!("Entered /Artifact marked content (no type specified)");
                    }
                }
            }
    ...
    fn process_xobject(&mut self, name: &str) -> Result<()> {
        ...
        let xobject_dict = match xobject.as_dict() {
            Some(d) => d,
            None => {
                tracing::debug!("XObject '{}' is not a dictionary", name);
                return Ok(());
            }
        };
        ...
        match subtype {
            Some("Form") => {
                tracing::debug!("Processing Form XObject: {}", name);
                ...
                if let Some(xobj_resources) = xobject_dict.get("Resources") {
                        ...
                        && let Some(res_dict) = res_obj.as_dict()
                    {
                        ...
                        if !has_font && !has_xobject {
                            tracing::debug!("Skipping Form XObject '{}': no Font/XObject in Resources", name);
            ...
            Some("Image") => {
                tracing::debug!("Skipping Image XObject: {}", name);
                Ok(())
            }
            _ => {
                tracing::debug!("Unknown XObject subtype for '{}': {:?}", name, subtype);
                Ok(())
            }
        }
    }
    ...
    fn insert_space_as_span(&mut self) -> Result<()> {
        ...
        tracing::trace!("PUSH space span with offset_semantic={}", span.offset_semantic);
    ...
    fn advances_along_writing_axis(start: Matrix, wmode: u8, e: f32, f: f32, font_size: f32) -> bool {
        if wmode != 0 {
            return true;
        }
