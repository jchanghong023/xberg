//! Content-stream re-serialization for destructive redaction.
//!
...
//!
//! ## Binary-safe string emission (security-relevant)
//!
...
fn serialize_remaining_operator(output: &mut Vec<u8>, op: &Operator) {
    match op {
        ...
        Operator::InlineImage { dict, data } => {
            ...
            // emission randomizes the content stream's bytes every run. ~keep
...
pub(crate) fn serialize_object(output: &mut Vec<u8>, obj: &Object) {
    match obj {
        ...
        Object::Dictionary(dict) => {
            ...
            // already sorts for this reason. ~keep
            let mut keys: Vec<_> = dict.keys().collect();
            keys.sort();
            for key in keys {
                if let Some(value) = dict.get(key) {
...
mod tests {
    ...
    fn printable_string_is_literal_with_three_escapes() {
        ...
        // ( ) \ escaped; everything else verbatim, literal form. ~keep
    ...
    fn binary_string_is_hexadecimal() {
        let mut out = Vec::new();
        // contains a NUL and a high byte ⇒ not literal-safe. ~keep
    ...
    fn control_bytes_force_hex_no_malformed_literal() {
        ...
        // parser and leave the secret grep-recoverable; hex form is safe. ~keep
        let mut out = Vec::new();
    ...
    fn tj_printable_unchanged_byte_for_byte() {
        // Regression: prior literal output must be preserved for ASCII. ~keep
        let mut out = Vec::new();
        serialize_operator(
            &mut out,
            &Operator::Tj {
