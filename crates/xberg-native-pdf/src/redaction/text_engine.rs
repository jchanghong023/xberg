//! Content-stream text-redaction engine — guarantees "no recoverable text"
//! and "no width/shift side channel".
//!
...
//! (a mis-composed matrix silently *under*-redacts a glyph that visually
//! sits in the region). Mitigations encoded here:
...
use super::text_prune::{Glyph, TextPruneResult, prune_run};
...
///
/// Implemented by the engine over the real `FontInfo`; a stub
...
pub trait FontMetrics {
    ...
    /// string (fail closed, never under-redact).
...
pub struct TextEngineResult {
    ...
    /// Distinct `(font_hash, code)` removed.
...
fn show_string(
    ...
    for (code, src) in decoded {
        ...
        // Advance the text matrix: tx = ((w0 - 0) Tfs + Tc + Tw?) Th. ~keep
...
/// (no `TJ` deltas — kills the width/shift side channel).
...
/// unredacted bytes are never persisted).
...
mod tests {
    use super::*;
    use crate::redaction::region::{DEFAULT_EDGE_PADDING, RedactionRegion, RegionSet};
    ...
    fn text_fully_in_region_is_removed_no_offset() {
        // 10pt font at (100,100); "SECRET" each glyph 5pt wide → x 100..130. ~keep
    ...
    fn straddle_splits_and_reanchors_absolutely() {
        ...
        // 119.5) so B clearly survives and x onward is removed. ~keep
        ...
        // first glyph's position (x=100), not a relative offset. ~keep
        let tm = out
            .operators
            .iter()
            .find_map(|o| match o {
    ...
    fn identity_h_middle_cid_removed_others_survive() {
        ...
        // exactly that CID's two bytes and keep CID1 and CID3 intact. ~keep
    ...
    fn identity_h_odd_length_show_is_refused_fail_closed() {
        ...
        // codes) must be refused rather than mis-decoded — fail closed. ~keep
    ...
    fn tj_array_with_offsets_redacted_drops_deltas() {
        // [(AB) -200 (CD)] TJ at (100,100). Region covers everything. ~keep
        let ops = vec![
            Operator::BeginText,
            Operator::Tf {
                font: "F1".into(),
    ...
    fn composite_font_with_regions_refuses_and_keeps_original() {
        // Fail-closed: a Type0/composite font we cannot
        // reliably decode must NOT be silently passed through as redacted. ~keep
        let ops = doc([1.0, 0.0, 0.0, 1.0, 100.0, 100.0], b"SECRET");
        ...
        assert_eq!(out.glyphs_removed, 0);
        // Original show emitted unchanged (caller discards on refusal). ~keep
    ...
    fn text_rendering_matrix_matches_spec_9_4_4() {
        ...
        // because x scales by Tfs·Th=12. ~keep
