...
pub fn classify_region(spans: &[TextSpan], indices: &[usize]) -> RegionClass {
    ...
    if line_count < 6 {
        // Headings, captions, single paragraphs — leave to default behaviour. ~keep
        return RegionClass::Mixed;
    }
    ...
    // load-bearing for the reorder gates. ~keep
    ...
    // of how wide the row spans. ~keep
    if mean_chars < 10.0 {
    ...
    // long enough to otherwise read as prose — from a real prose body. ~keep
    if form_frac >= 0.4 {
        return RegionClass::Form;
    }

    // REFERENCE: numbered entries, or a hanging-indent two-level left edge, with
    // enough text per line to exclude table cells. Treated like prose downstream. ~keep
    ...
    // Mixed = prior behaviour, which is safe). ~keep
...
fn cluster_lines(spans: &[TextSpan], indices: &[usize], med_h: f32) -> Vec<LineStat> {
    ...
    // Keep `span_lefts`/`span_rights` paired and left-sorted for gap analysis. ~keep
    for l in &mut lines {
...
fn has_hanging_indent(left_edges: &[f32], med_h: f32) -> bool {
    ...
    // Both bands must be substantial (≥ 25% of lines each). ~keep
...
mod tests {
    ...
    fn classify_table_cells_is_table() {
        // 12 short numeric cells across 6 rows, narrow → mean_chars < 8. ~keep
        let mut spans = Vec::new();
        for r in 0..6 {
