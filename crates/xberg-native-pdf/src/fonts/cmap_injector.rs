...
fn finalize_sfnt(sfnt_version: u32, mut tables: Vec<TableEntry>) -> Vec<u8> {
    ...
    //   3. checkSumAdjustment = 0xB1B0AFBA - sum_total ~keep
...
/// table — widths live in the document's `/W` array. When xberg-native-pdf
...
pub fn inject_hmtx(font_bytes: &[u8], widths_by_gid: &HashMap<u16, u16>) -> Option<Vec<u8>> {
    ...
    // widths_by_gid is in 1/1000 em (PDF /W units); convert to font design units. ~keep
    ...
    for gid in 0u16..num_glyphs as u16 {
        ...
        new_hmtx.extend_from_slice(&[0u8, 0u8]);
    }
    ...
    // Patch hhea.numberOfHMetrics (u16 at offset 34) to numGlyphs. ~keep
...
fn coalesce_cmap_segments(pairs: &[(u16, u16)]) -> Vec<Segment> {
    ...
    // to glyph 0 (delta = 1 makes glyph index = 0xFFFF + 1 = 0). ~keep
...
fn write_format4_subtable(segs: &[Segment]) -> Vec<u8> {
    ...
    // PDF→DOCX→PDF round-trips of MicrosoftSansSerif-subset PDFs.) ~keep
    let subtable_size = 16 + 8 * seg_count;
    let total_size = 4 + 8 + subtable_size;
    let mut buf = Vec::with_capacity(total_size);

    write_be_u16(&mut buf, 0);
    write_be_u16(&mut buf, 1);

    write_be_u16(&mut buf, 0);
    write_be_u16(&mut buf, 3);
    write_be_u32(&mut buf, 12);

    write_be_u16(&mut buf, 4);
    write_be_u16(&mut buf, subtable_size as u16);
    write_be_u16(&mut buf, 0);
    write_be_u16(&mut buf, seg_count_x2);
...
mod tests {
    ...
    fn minimal_otf() -> Vec<u8> {
        ...
        // table-directory rewriting code paths. ~keep
        let mut buf = Vec::new();
