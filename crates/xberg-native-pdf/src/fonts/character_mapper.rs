...
impl CharacterMapper {
    ...
    ///
    /// ```
    /// use xberg_native_pdf::fonts::character_mapper::{CharacterMapper, PredefinedCMapConfig};
    ///
    ...
    pub fn map_character(&self, code: u32) -> Option<String> {
        ...
        // whose CIDs are insertion-ordered (not codepoint-aligned). ~keep
        ...
        if let Some(glyph_name) = self.code_to_glyph_name(code)
            && let Some(unicode_str) = self.map_glyph_name_internal(&glyph_name)
        {
        ...
        // look up the Unicode code point for this CID. ~keep
        if let Some(ref config) = self.predefined_cmap
            && let Some(unicode_str) = self.lookup_predefined_cmap(config, code)
        {
            return Some(unicode_str);
        }

        // Priority 4: ActualText (handled externally in BDC operator / structure tree) ~keep

        if let Some(ref encoding) = self.font_encoding
            && let Some(&ch) = encoding.get(&code)
        {
            return Some(ch.to_string());
        }

        // No mapping found - return U+FFFD replacement character per PDF Spec 9.10.2 ~keep
    ...
    fn lookup_predefined_cmap(&self, config: &PredefinedCMapConfig, code: u32) -> Option<String> {
        // Truncate to u16 for CID lookup (CIDs are 16-bit values) ~keep
        let cid = code as u16;
        ...
        let unicode_codepoint = match config.ordering.as_str() {
            ...
            // Arabic block; without it these decode as Latin-Extended-B garbage. ~keep
...
/// before falling back to CID-as-Unicode.
pub(crate) fn glyph_name_to_unicode(glyph_name: &str) -> Option<String> {
    ...
    // again — the base codepoint is what we want for extraction. ~keep
    if let Some(dot_idx) = glyph_name.find('.') {
        let base = &glyph_name[..dot_idx];
        if !base.is_empty()
            && let Some(&ch) = ADOBE_GLYPH_LIST.get(base)
        {
            return Some(ch.to_string());
        }
    }
    ...
    {
        // Reject surrogates and control chars (unprintable as text). ~keep
        if !c.is_control() {
...
impl CharacterMapper {
    ...
    fn glyph_name_windows1252_c1_controls(code: u32) -> Option<String> {
        match code {
            ...
            0x83 => Some("florin".to_string()),
            0x84 => Some("quotedblbase".to_string()),
            0x85 => Some("ellipsis".to_string()),
            0x86 => Some("dagger".to_string()),
            0x87 => Some("daggerdbl".to_string()),
            0x88 => Some("circumflex".to_string()),
            0x89 => Some("perthousand".to_string()),
            0x8A => Some("Scaron".to_string()),
            0x8B => Some("guilsinglleft".to_string()),
            0x8C => Some("OEligature".to_string()),
            0x8D => None,
            0x8E => Some("Zcaron".to_string()),
            0x8F => None,

            0x90 => None,
            0x91 => Some("quoteleft".to_string()),
            0x92 => Some("quoteright".to_string()),
            0x93 => Some("quotedblleft".to_string()),
            0x94 => Some("quotedblright".to_string()),
            0x95 => Some("bullet".to_string()),
            0x96 => Some("endash".to_string()),
            0x97 => Some("emdash".to_string()),
            0x98 => Some("tilde".to_string()),
            0x99 => Some("trademark".to_string()),
            0x9A => Some("scaron".to_string()),
            0x9B => Some("guilsinglright".to_string()),
            0x9C => Some("oeligature".to_string()),
            0x9D => None,
            0x9E => Some("zcaron".to_string()),
            0x9F => Some("ydieresis".to_string()),

            0xA0 => Some("space".to_string()),
            0xA1 => Some("exclamdown".to_string()),
            0xA2 => Some("cent".to_string()),
            0xA3 => Some("sterling".to_string()),
            0xA4 => Some("currency".to_string()),
            0xA5 => Some("yen".to_string()),
            0xA6 => Some("brokenbar".to_string()),
            0xA7 => Some("section".to_string()),
            0xA8 => Some("dieresis".to_string()),
            0xA9 => Some("copyright".to_string()), // © (U+00A9) - COMMON: copyright
            0xAA => Some("ordfeminine".to_string()),
            0xAB => Some("guillemotleft".to_string()),
            0xAC => Some("logicalnot".to_string()),
            0xAD => Some("hyphen".to_string()),
            0xAE => Some("registered".to_string()),
            0xAF => Some("macron".to_string()),
            0xB0 => Some("degree".to_string()),
            0xB1 => Some("plusminus".to_string()),
            0xB2 => Some("twosuperior".to_string()),
            0xB3 => Some("threesuperior".to_string()),
            0xB4 => Some("acute".to_string()),
            0xB5 => Some("mu".to_string()),
            0xB6 => Some("paragraph".to_string()),
            0xB7 => Some("periodcentered".to_string()),
            0xB8 => Some("cedilla".to_string()),
            0xB9 => Some("onesuperior".to_string()),
            0xBA => Some("ordmasculine".to_string()),
            0xBB => Some("guillemotright".to_string()),
            0xBC => Some("onequarter".to_string()),
            0xBD => Some("onehalf".to_string()),
    ...
    fn glyph_name_windows1252_latin_supplement(code: u32) -> Option<String> {
        match code {
            ...
            0xC3 => Some("Atilde".to_string()),
            0xC4 => Some("Adieresis".to_string()),
            0xC5 => Some("Aring".to_string()),
            0xC6 => Some("AEligature".to_string()),
            0xC7 => Some("Ccedilla".to_string()),
            0xC8 => Some("Egrave".to_string()),
            0xC9 => Some("Eacute".to_string()),
            0xCA => Some("Ecircumflex".to_string()),
            0xCB => Some("Edieresis".to_string()),
            0xCC => Some("Igrave".to_string()),
            0xCD => Some("Iacute".to_string()),
            0xCE => Some("Icircumflex".to_string()),
            0xCF => Some("Idieresis".to_string()),
            0xD0 => Some("Eth".to_string()),
            0xD1 => Some("Ntilde".to_string()),
            0xD2 => Some("Ograve".to_string()),
            0xD3 => Some("Oacute".to_string()),
            0xD4 => Some("Ocircumflex".to_string()),
            0xD5 => Some("Otilde".to_string()),
            0xD6 => Some("Odieresis".to_string()),
            0xD7 => Some("multiply".to_string()),
            0xD8 => Some("Oslash".to_string()),
            0xD9 => Some("Ugrave".to_string()),
            0xDA => Some("Uacute".to_string()),
            0xDB => Some("Ucircumflex".to_string()),
            0xDC => Some("Udieresis".to_string()),
            0xDD => Some("Yacute".to_string()),
            0xDE => Some("Thorn".to_string()),
            0xDF => Some("germandbls".to_string()),
            0xE0 => Some("agrave".to_string()),
            0xE1 => Some("aacute".to_string()),
            0xE2 => Some("acircumflex".to_string()),
            0xE3 => Some("atilde".to_string()),
            0xE4 => Some("adieresis".to_string()),
            0xE5 => Some("aring".to_string()),
            0xE6 => Some("aeligature".to_string()),
            0xE7 => Some("ccedilla".to_string()),
            0xE8 => Some("egrave".to_string()),
            0xE9 => Some("eacute".to_string()),
            0xEA => Some("ecircumflex".to_string()),
            0xEB => Some("edieresis".to_string()),
            0xEC => Some("igrave".to_string()),
            0xED => Some("iacute".to_string()),
            0xEE => Some("icircumflex".to_string()),
            0xEF => Some("idieresis".to_string()),
            0xF0 => Some("eth".to_string()),
            0xF1 => Some("ntilde".to_string()),
            0xF2 => Some("ograve".to_string()),
            0xF3 => Some("oacute".to_string()),
            0xF4 => Some("ocircumflex".to_string()),
            0xF5 => Some("otilde".to_string()),
            0xF6 => Some("odieresis".to_string()),
            0xF7 => Some("divide".to_string()),
            0xF8 => Some("oslash".to_string()),
            0xF9 => Some("ugrave".to_string()),
            0xFA => Some("uacute".to_string()),
            0xFB => Some("ucircumflex".to_string()),
            0xFC => Some("udieresis".to_string()),
            0xFD => Some("yacute".to_string()),
            0xFE => Some("thorn".to_string()),
            0xFF => Some("ydieresis".to_string()),
...
mod internal_tests {
    ...
    // ==========================================================================
    // Adobe Glyph List §6 + synthetic-name patterns
    // ==========================================================================
    ...
    // subset fonts without a `CIDToGIDMap`. ~keep

    #[test]
    fn glyph_name_to_unicode_agl_exact_match() {
        // Canonical AGL entries — the bullet/ligature cases that motivate this fallback. ~keep
    ...
    fn glyph_name_to_unicode_uniXXXX_synth() {
        // Per AGL §6, `uni` + 4 hex digits = U+XXXX. ~keep
    ...
    fn glyph_name_to_unicode_unknown_or_control() {
        ...
        assert_eq!(glyph_name_to_unicode("uni0007"), None);
    }
    ...
    // ===== Tests for Predefined CMap Support ===== ~keep
    ...
    fn test_predefined_cmap_japan1_ascii() {
        ...
        // For CIDs outside ASCII range, it falls through to Priority 3. ~keep
    ...
    // Regression test: subset Type0 font with Identity-H and an
    ...
    // by a code-as-ASCII or CID-as-Unicode heuristic. ~keep
    #[test]
    ...
    // fix above only activates when a ToUnicode CMap is attached. ~keep
    #[test]
