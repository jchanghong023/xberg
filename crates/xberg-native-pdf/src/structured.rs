//! Structured per-page extraction (`extract_structured`).
//!
...
pub struct StructuredRegion {
    ...
    /// keeps one `section_id` (§5/§6). `None` for untagged PDFs or content
    /// outside any section.
...
fn role_for_span(span: &TextSpan) -> RegionRole {
    if let Some(at) = &span.artifact_type {
        return match at {
            ...
            // Watermark / Other pagination, plus Layout / Page / Background. ~keep
...
/// split (KJF reference Bible, where the merged book-title line
...
fn detect_gutter_x(body: &[&TextSpan], page_width: f32) -> Option<f32> {
    ...
    // index at the jump is the left-column count. ~keep
...
/// Column-detection mode for `build_structured_page_with_mode` (Fix 3). The
/// geometric column split is heuristic — ISO 32000-1:2008 defines
...
/// (Fix 3). `Auto` runs the gutter heuristic; `Two` forces a split
...
/// (ISO 32000-1:2008 §14.8.4 — §4/§5/§6). For untagged PDFs the
...
pub(crate) fn build_structured_page_full(
    ...
    for span in spans {
        ...
        // authoritatively a label/numeral (§14.8.4.3.3 — §4). ~keep
        let mcid = span.mcid;
...
mod tests {
    ...
    /// Faithful reproduction of the KJF reference-Bible page 11
    ...
    fn issue_734_kjf_reference_columns_get_indices() {
        ...
        // gutter ≈ [197,230]. Line-level spans (≈140 pt wide left lines). ~keep
        let mut spans = Vec::new();
        // Full-width book title + running header merged into body flow. ~keep
        spans.push(span("Le Troisième Livre de Moïse Appelé GENÈSE", 57.6, 720.0, 339.0));
        // Gutter-centred page number that escaped artifact classification. ~keep
        spans.push(span("1", 224.6, 706.0, 8.0));

        let mut y = 690.0;
        // Left column: verses 1..=6, two wrapped line-spans each, per-verse MCID. ~keep
        for v in 1..=6u32 {
            spans.push(span_mcid(
                ...
                y,
                139.0,
                4 + v, // verse 1 → mcid 5 ~keep
            ));
            spans.push(span_mcid("terre.", 57.6, y - 14.0, 28.0, 4 + v));
            y -= 30.0;
        }
        // Right column: verses 14..=19, two wrapped line-spans each, per-verse MCID. ~keep
        y = 690.0;
        for v in 14..=19u32 {
            spans.push(span_mcid(
        ...
        // column index, not be forced to one side by its centre. ~keep
        let title = page
            .regions
            .iter()
    ...
    fn two_column_body_gets_column_indices() {
        // Left column at x≈60, right column at x≈360 on a 612-wide page. ~keep
        let spans = vec![
    ...
    fn narrow_gutter_short_line_columns_get_indices() {
        ...
        // gutter ≈ [206,226] (20pt, ~4.6 % of width). Word-level spans. ~keep
        let pw = 432.0;
        let mut spans = Vec::new();
        let mut y = 700.0;
        for row in 0..6 {
    ...
    /// gutter). The split falls back to the page midpoint (Fix 3).
    #[test]
    ...
    /// split — every region gets `column_index == None` (Fix 3).
    #[test]
    fn column_mode_single_suppresses_clear_columns() {
        ...
        // Sanity: Auto splits this layout (mirrors two_column_body_gets_column_indices). ~keep
        let auto = build_structured_page_with_mode(0, pw, 792.0, spans.clone(), ColumnMode::Auto);
