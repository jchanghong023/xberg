...
pub fn detect_dense_single_line(glyphs: &[DetectorGlyph]) -> bool {
    ...
    // dense single-line text the dominant bin matches the old clustering. ~keep
    ...
    // one large gap stands out from the rest. ~keep
...
pub fn detect_narrow_tracked(glyphs: &[DetectorGlyph]) -> bool {
    ...
    // Sort an index vector by X instead of cloning the glyph slice. ~keep
    ...
    // typically ~0.05-0.1 × font_size. For monospace, ~0.0. ~keep
...
mod tests {
    ...
    fn sub_super_detects_subscript_y_offset() {
        ...
        // displacement of 4pt with font_size=10pt → 0.4×fs). ~keep
        let glyphs = vec![
            glyph(50.0, 100.0, 5.0, 10.0),
    ...
    fn narrow_tracked_detects_stretched_justification() {
        ...
        // here are ~3pt (justified stretch). ~keep
        let mut glyphs = Vec::new();
        for i in 0..10 {
