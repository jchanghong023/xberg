...
impl ResolutionPipeline {
    ...
    pub(crate) fn resolve<'a>(
        ...
        // the composer where both are available. ~keep
...
fn apply_inks_selector_override(
    ...
    match arr.get(1).and_then(|o| o.as_name()) {
        ...
        Some(spot_name) => {
            ...
            // per-plate backend can pick the right routing per-surface. ~keep
            ...
            // through to `alt_cmyk_fallback` (device doesn't). ~keep
...
mod tests {
    ...
    fn pipeline_resolves_device_gray_path_fill() {
        ...
        // Colour: Gray(0.25) folded with fill_alpha=0.8 → Rgba(0.25, 0.25, 0.25, 0.8). ~keep
        match cmd.color {
    ...
    fn pipeline_resolves_spaced_separation_with_type4_end_to_end() {
        ...
        // clip plan) doesn't interfere. ~keep
        ...
        // magenta corner #EC008C = (0.9255, 0, 0.5490). ~keep
